      ...
      <div class="transfer__body">
        ...
        <div class="transfer-pane">
          ...
          <el-scrollbar class="transfer-pane__body shadow right-pane">
            ...
            <template v-for="type in tabKeys" v-if="!selectedData[type].length">
             <div
...
export default {
  ...
  methods: {
    ...
    filterNode(value, data) {
      ...
      return this.getNodeProp(data, 'label').indexOf(value) !== -1;
    },
    onCheckChange (data, checked, tabKey) {
      // console.log(data, checked, tabKey, '1111111');
      ...
      this.$nextTick(() => {
        ...
        const sing = this.selectedData[tabKey].findIndex(t => t.nodeId === data.nodeId);
        if(checked) {
          if(sing == -1) {
            this.selectedData[tabKey].push(data)
          }
        }else {
          if(sing > -1) {
            this.selectedData[tabKey].splice(sing, 1)
          }
        }
        // this.$set(this.selectedData, this.activeTabName,nodes );
        // console.log(2,this.selectedData,this.activeTabName,nodes)
        this.isNumEnough();
        this.$forceUpdate()
      })
    },
    ...
    confirm () {
      const res = {}
      for (const type of this.tabKeys) {
        res[type] = this.unique(this.selectedData[type].concat(this.aloneCheckedData[type]))
      }
    ...
    dataInit(){
      ...
      this.tabList.forEach(item => {
        console.log('tabKeyItem', item, this.value, this.tabConfig)
      ...
      const setKey = () => {
        this.loading = false
        let elChild = this.$refs[this.activeTabName]
        let keys = []
        this.value[this.activeTabName].forEach(nodes => {
          keys.push(nodes.nodeId)
        })
        if (!elChild) {
          return false
        }
        elChild.forEach(el => {
          setTimeout(() => {
            console.log('setCheckedNodes', keys)
      ...
      console.log('value', this.value, this.tabKeys, this.activeTabName);
      
      // 获取用户
      if (this.activeTabName === 'user' || this.activeTabName.includes('user')) {
        this.$client.getUserEmps({ is_contain_emp: true, show_all: true})
          .then((res) => {
            console.log('用户',res, this.value);
      ...
      if (this.activeTabName === 'role') {
        ...
        // this.$client.getRoleTree({is_show: 1})
        this.$client.getRoleTree_center({tenant_id: window.newHttpConfig.TenantId})
          .then(res => {
            this.loading = false;
            console.log('角色',res)
            if (res.errcode == "200") {
              this.treeData = res.data.childs
              this.dataConversion(this.treeData)
              this.loading = false;
            } else {
              this.loading = false;
            }
      ...
      if (this.activeTabName === 'position_job') {
        this.$client.GetAllPositionDataBase()
          .then(res => {
            this.loading = false;
            console.log('职务',res)
            if (res.head.result === "200") {
              ...
              res.context.forEach(item => {
                list.push(...item.posttions_jobs)
              })
              list.filter(function(item,index){
                return list.indexOf(item) === index;  // 因为indexOf 只能查找到第一个  
              }).forEach((item, ind) => {
                arr.push({i_id: ind, c_name: item, type: 4, position_job: item})
              })
              this.treeData = [
                {i_id: '100000', c_name: '职务组', type: 1, children: arr}
              ]
              // console.log(list,children, this.treeData);
              this.loading = false;
            } else {
              this.loading = false;
            }
    ...
    dataConversion(list) {
      list.forEach(item => {
        ...
        if (item.children && item.children.length > 0) {
          this.dataConversion(item.children)
        }
      })
    },
  ...
  watch: {
    searchString (newVal) {
      ...
      // this.debounceSearch()
      console.log(this.$refs[this.activeTabName], this.activeTabName);
      this.$refs[this.activeTabName].forEach(el => {el.filter(newVal)})
    },
