...
const defaultOption = {
  ...
  nodeId: function ( data ) {
    return data.hasOwnProperty( 'userId' ) ? data.userId : data.hasOwnProperty( 'i_id' ) ? data.i_id : data.deptId
  },
  // 生成节点的名称 可选值 string | function
  label: function ( data, node ) {
    return data.hasOwnProperty( 'userId' ) ? data.userName : data.hasOwnProperty( 'i_id' ) ? data.c_name : data.deptName
  },
  // 判断是否为叶子节点 可选值 string | function
  isLeaf: function ( data, node ) {
    // console.log(data, node, 'isleaf')
    return data.hasOwnProperty( 'type' ) ? data.type === 0 : data.hasOwnProperty( 'isleaf' ) ? data.isleaf === 1 : false          // 含有empID为人员  且为叶子节点
  },
  ...
  disabled: function ( data, node ) {
    // console.log(data, node, 'disabled')
    return false
  },
  ...
  onsearch: async function ( searchString, resolve, reject ) {
    ...
    // console.log(searchString, '搜索', ( await GET_PAGE_EMPLOYEE() ).data)
    resolve( ( await GET_PAGE_EMPLOYEE() ).data )
  }
}

export const DEP_CONFIG = Object.assign( {}, defaultOption )
export const JOB_CONFIG = Object.assign( {}, defaultOption, { tabKey: 'position_job', tabName: '职务', disabled: ( data, node ) => data.hasOwnProperty( 'type' ) ? data.type === 1 : false })
export const ROLE_CONFIG = Object.assign( {}, defaultOption, { tabKey: 'role', tabName: '角色', disabled: ( data, node ) => data.hasOwnProperty( 'type' ) ? data.type === 1 : false })
export const USER_CONFIG = Object.assign( {}, defaultOption, { tabKey: 'user', tabName: '指定人员', onload: loadUserData, disabled: ( data, node ) => data.hasOwnProperty( 'type' ) ? data.type === 1 : false } )
...
export const CONFIG_LIST = [DEP_CONFIG,JOB_CONFIG, ROLE_CONFIG, USER_CONFIG, DEP_USER_CONFIG, POSITION_CONFIG]
