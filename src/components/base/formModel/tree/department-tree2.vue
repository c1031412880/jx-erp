...
export default {
  ...
  watch: {
    ...
    value(val) {
      if (val) {
        if(!this.isShowCheckbox && this.isFirstSelect && this.data.length > 0) {
  ...
  methods: {
        ...
        .then((res) => {
          ...
          if (res.head.result === "200") {
            ...
            if(this.data.length > 0) {
              this.defaultKeys.push(this.data[0].index)
            }
            if (this.value !== '') {
    ...
    weedingSubsets(data,value) {
      data.forEach((item,index) =>{
        if (item.index == value || item.parent_id == value) {
          data.splice(index,1)
    ...
    handleNodeClick(data) {
      ...
      if(!this.isShowCheckbox) {
        // 获取当前节点父级节点
        let parentNode = this.outGroupTreeNode(this.data, data, 'index');
    ...
    // 获取当前节点的父级节点
    outGroupTreeNode(nodes,node, key = 'index') {
      let groupNode  = null
      const out = (as, a) => {
    ...
    resetChecked() {
      ...
      this.userSelect = [];
      if(this.data.length > 0) {
        this.defaultKeys = [this.data[0].index];
      }
      this.isFirstSelect = true
    },
  },
