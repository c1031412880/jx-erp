...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.loading = true;
      this.$client
        .getDeptTreeWarehouse(params)
        .then((res) => {
          // console.log(JSON.stringify(res));
          if (res.head.result === "200") {
            this.data = res.context;
            if(this.data.length > 0) {
              this.defaultKeys.push(this.data[0].node_id)
            }
    ...
    // Id回显
    setSelectCheckedKeys(node_id) {
      this.$refs.tree.setCurrentKey(node_id)
      setTimeout(() => {
        let node = this.$refs.tree.getCurrentNode(node_id)
        if (node) {
    ...
    resetChecked() {
      ...
      this.curSelectNode = [];
      if(this.data.length > 0) {
        this.defaultKeys = [this.data[0].node_id];
      }else{
        this.defaultKeys = []
      }
    },
