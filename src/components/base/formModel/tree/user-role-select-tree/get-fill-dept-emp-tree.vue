  ...
  methods: {
      ...
      this.$client
        ...
        .then((res) => {
          ...
          if (res.head.result === '200') {
            ...
            // console.log('刷新的',this.id, this.data, this.indexObj, this.idObj)
            this.setCheckedKeys(this.id)
            this.setCurrentKey(this.id)
          }
        })
    ...
    onCheck(data, nodes) {
      ...
      if(this.isShowCheckbox) {
        ...
        // console.log(data, nodes, curSelectNode);
      }
      if(this.isShowCheckbox) {
        let curSelectNode = this.$refs.tree.getCheckedNodes();
        ...
        // this.$emit()
        let isCheck = false;
        if(curSelectNode.length){
          for (let i=0;i<curSelectNode.length;i++){
            let it = curSelectNode[i];
            if(data.i_id === it.i_id){
              isCheck = true
              break;
            }
          }
        ...
        this.$emit("on-check", nodes, keys, data, isCheck);
        this.outNodesName()
      }
    },
    // 设置勾选节点
