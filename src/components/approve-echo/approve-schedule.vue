      ...
      <div class="comment-div-dialog" style="height:300px; overflow-y: auto;">
        <el-form ref="countersignForm" size="mini" :model="countersignForm" :rules="countersignRules"  label-width="150px">
          ...
          <el-form-item label="加签审批人:" prop="add_user_ids">
            ...
            <el-select style="width: 100%" v-model="countersignForm.add_user_ids" multiple clearable filterable placeholder="请选择">
              <el-option
                v-for="item in usersListOptions"
                :key="item.value"
                :label="item.label"
                :value="item.value">
              </el-option>
          ...
          <el-form-item label="" prop="">
           <span style="color:#909399;font-size:12px;">说明:选择多人时默认为依次审批流程</span>
          </el-form-item>
          ...
          <el-form-item label="加签备注:" prop="sign_remark" v-if="countersignForm.add_type === 2">
            <el-input type="textarea" v-model="countersignForm.sign_remark" placeholder="请输入加签备注"></el-input>
          </el-form-item>
        </el-form>
      </div>
  ...
  methods: {
      ...
      if (this.isFlow) {
        ...
        this.$client.GetReviewProcessDocumentFlow({
          ...
          if(res.head.result === '200') {
            ...
            this.$emit('out-data', this.data, obj)
            let bool = true
    ...
    checkShowApproveUseName(item,index) {
     ...
     if(item.oper_type == 3) {
       showApproveUseName = ''
        item.users.forEach((zItem,zIndex) => {
            showApproveUseName +=  `${zItem.name}(${!zItem.read ? '未读' : '已读'})+ `
        });
