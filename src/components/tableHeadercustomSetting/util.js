 ...
 * @param {String} OnlyKey  根据唯一key 做合并
 * @param {Boolean} isRepeat  根据OnlyKey生成相同的合并数列isRepeat
 */
 export function handleTableSpan(mergeKeys, tableData, OnlyKey, isRepeat = false) {
    const spanObj = {}
    if (mergeKeys instanceof Array && tableData instanceof Array && mergeKeys.length && tableData.length) {
        ...
        if (isRepeat) {
            Object.keys(spanObj).forEach(key => {
                spanObj[key] = spanObj[OnlyKey]
            })
        }
        return spanObj
    }
 ...
 * @param {Boolean} notEmpty 是否要合并有 ’ ‘ 空单元格，此参数非必传。默认不合并, notEmpty = true 则合并
 */
