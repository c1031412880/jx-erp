  ...
  >
    <div class="draggable">
      ...
      >
        ...
        >
          <template v-if="item.prop == 'i_sort'">
...
export default {
  ...
  watch: {
    dialogBool(bool) {
      if (bool === false) {
        ...
        // alert(JSON.stringify(this.formOrderSettingInfo))
  ...
  methods: {
    ...
    // 初始化数据
    configTableTitle() {
      this.formOrderSettingInfo.forEach((item, index) => {
        let obj = {
          i_id: item.i_id,
          i_classify_id: item.i_classify_id,
          label: item.c_form_name,
          i_sort: index 
        };
    ...
    updatePlaybill() {
      ...
      this.$parent.updateFormOrde(this.tableData);
      this.dialogClose();
    },
