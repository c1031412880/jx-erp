...
export default {
  ...
  methods:{
    ...
    setTreeData(val) {
      ...
      this.handle(this.newData, val)
      this.manuTree = this.newData
    },
    handle(data, val) {
      data.forEach(item => {
        item.disabled = val.indexOf(item.c_privilege) === -1 ? true : false
        if(item.children && item.children.length) this.handle(item.children, val)
      })
    },
  },
  watch:{
