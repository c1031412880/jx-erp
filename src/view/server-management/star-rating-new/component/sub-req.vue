  ...
  <div>
    <el-form :inline="true" size="mini">
      ...
      <el-form-item label="是否特殊">
        <el-select v-model="form.special" clearable placeholder="请选择">
          <el-option label="特殊线路驾驶员" :value="1"></el-option>
          <el-option label="特殊驾驶员" :value="2"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item>
        <el-checkbox v-model="form.is_leave" :true-label="0" :false-label="1">
          仅显示在职驾驶员
        </el-checkbox>
      </el-form-item>
      <el-form-item>
        <el-checkbox v-model="form.is_cancle_star" :true-label="1" :false-label="2">
          是否取消本季度星级考核资格
        </el-checkbox>
      </el-form-item>
      <el-form-item>
      ...
      <el-form-item>
        <el-button type="primary" @click="onBtn('rebuild')">重新生成</el-button>
        <!-- v-if="$isPowerShow('rebuild')"  -->
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="onBtn('add')">批量提交审批</el-button>
...
export default {
  ...
  methods: {
    ...
    searchTimeChange(value) {
      if (value) {
        this.form.year = value.slice(0, 4) * 1
        this.form.quarter = value.slice(-2) * 1
      } else {
        this.form.year = ''
        this.form.quarter = ''
      }
