  ...
  <div style="height:60vh; overflow-x: hidden">
    <el-row style="margin-bottom:40px">
      <el-col :span="11" style="height:300px;width:600px">
        ...
        <img-box ref="laborInfo"
          :imgData='VehicleLicenseData' 
          :imageHeight="310" 
          :action="'/api/ERPWeb/DataBase/UploadFile'" 
          :ImageBoxName="VehicleLicense"
          :picturePathList="laborInfoPicturePathList" 
          :isShowRemark="true" 
          :disabled="this.curChangeType == 'detail'" 
          
          @on-add-img ='addVehLicenseImg(arguments)' 
          @on-del ='deleteImg(arguments)'  @on-del-all ='deleteAllImg' @send-remark="saveRemark"></img-box>
      ...
      this.$client.getOrderFile({order_id:val.id}).then(res => {
        if(res.head.result == "200") {
          ...
          res.context.forEach(element => {
            this.laborInfoPicturePathList.push(element.url)
          });
          console.log(this.laborInfoPicturePathList,"666")
          res.context.forEach(element => {
            if(element.type == 1){
            }else{
