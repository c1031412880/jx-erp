<template>
  <tr-dialog :visible="dialogBool" :title="showTitle" @before-close="dialogClose" width="650px"  :append-to-body="appendToBody">
    <div>
      <el-tabs v-model="activeName">
        ...
        <el-tab-pane label="安装车辆" name="second">
          <install-info ref="installInfo" :curChangeType="curChangeType" :containerVehicleRecode="containerVehicleRecode"></install-info>
        </el-tab-pane>
        <el-tab-pane label="修补翻新情况" name="third" v-if="curChangeType == 'detail'">
    ...
    resetData(){
      setTimeout(() => {
        ...
        if(this.curChangeType == 'detail') {
          this.$refs['maintainCondition'].resetData()  // 修补翻新信息清除
