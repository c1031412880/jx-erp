<template>
  <div class="vehicle-electric-entry">
    <sub-req
      ref="head"
      @on-add="openAddPage"
      @on-ok="onSubmit"
...
export default {
  ...
  data() {
    return {
      ...
      selectedIds:[],
      FileData: {
        url: window.newHttpConfig.URL_EASYMOCK +  "/Files/LocalFiles/车辆电耗录入模板.xlsx",
      },
  ...
  methods: {
    ...
    onSubmitExcel(exportInfo) {
      ...
      let params = formData;
      // console.log(params)
      this.$client.ImportVehicleChargeEnter(params)
      .then(res => {
    ...
    exportExcel(){
      ...
      this.$client.ExportVehicleChargeEnter(params).then(res => {
        if(res.head.result == '200') {
    ...
    loadData() {
      ...
      this.$client.ListVehicleChargeEnter(params)
        .then(res => {
          if (res.head.result == "200") {
            this.loading = false;
            ...
            this.sumTotal = {
              6: res.context.summary['val'],
              7: res.context.summary['service_charge'],
              8: res.context.summary['service_fee'],
              9: res.context.summary['total_fee'],
            }
          } else {
            this.loading = false;
            this.$message({
    ...
    saveAddEdit(form,type) {
      ...
      this.$client
        .AddOrEditVehicleChargeEnter(parmas)
        .then((res) => {
          if (res.head.result == "200") {
            this.loadData();
            if(type == 'addClose') {
    ...
    deleteRowInfo(info) {
      ...
      this.$client.BatchDeleteVehicleChargeEnter(params)
      .then(res => {
        if (res.head.result == "200") {
          this.loadData();
          this.$message({
