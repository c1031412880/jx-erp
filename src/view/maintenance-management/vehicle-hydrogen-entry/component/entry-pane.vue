<template>
  <div class="vehicle-hydrogen-entry">
    <sub-req
      ref="head"
      @on-add="openAddPage"
...
export default {
  ...
  methods: {
    ...
    onSubmitExcel(exportInfo) {
      ...
      let params = formData;
      this.$client.ImportVehicleHydrogenRecord(params).then((res) => {
    ...
    exportExcel() {
      ...
      this.$client.ExportVehicleHydrogenRecord(params).then((res) => {
        if (res.head.result == "200") {
    ...
    loadData() {
      ...
      this.$client
        .ListVehicleHydrogenRecord(params)
        .then((res) => {
          if (res.head.result == "200") {
            this.loading = false;
            ...
            this.sumTotal = {
              8: res.context.summary["val_total"],
              9: res.context.summary["amount_total"],
            };
          } else {
            this.loading = false;
            this.$message({
    ...
    verifyDataExists(form, type) {
      ...
      if (this.curSelecedType == "add") {
        ...
        //如果是添加则校验数据是否存在
        this.$client
          .VerifyVehicleHydrogenRecordExist(parmas)
          .then((res) => {
            if (res.head.result == "200") {
    ...
    saveAddEdit(parmas, type) {
      ...
      this.$client
        .AddOrEditVehicleHydrogenRecord(parmas)
        .then((res) => {
          if (res.head.result == "200") {
            this.loadData();
            if (type === 1) {
    ...
    openAddPage() {
      ...
      this.informationDialogTitle = "氢气录入";
    ...
    deleteRowInfo(info) {
      ...
      this.$client
        .BatchDeleteVehicleHydrogenRecord(params)
        .then((res) => {
          if (res.head.result == "200") {
            this.loadData();
            this.$message({
    ...
    saveBatch(list) {
      this.$client
        .VehicleHydrogenBatchInsert({ context: list })
        .then((res) => {
          if (res.head.result == "200") {
            this.loadData();
