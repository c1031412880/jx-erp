...
export default {
    ...
    return {
      ...
      FileData: {
        url:
          ...
          "/Files/TempFiles/车辆氢气消耗明细模板.xlsx",
      },
  ...
  methods: {
    ...
    onSubmitExcel(exportInfo) {
      ...
      let params = formData;
      this.$client.ImportVehicleHydrogenDetail(params).then((res) => {
    ...
    exportExcel() {
      ...
      this.$client.ExportVehicleHydrogenDetail(params).then((res) => {
        if (res.head.result == "200") {
    ...
    loadData() {
      ...
      this.$client
        .GetVehicleHydrogenDetailList(params)
        .then((res) => {
          if (res.head.result == "200") {
            this.loading = false;
