<template>
  <div class="engine-model-management">
    <sub-req
      ref="head"
      @on-add="openAddJobPage"
      @on-ok="onSubmit"
      @delete-Job="deleteEngine"
    ></sub-req>
    <tr-table
      :selectionShow="true"
      :data="data"
      ...
      @on-select-change-row="selectedEngineList"
    >
    ...
    <information-dialog
      ...
      @save-add-edit-engine="saveAddEditEngine"
    ></information-dialog>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.$client.getEngineModelByPage(params)
        .then(res => {
    ...
    openAddJobPage(type) {
      ...
      this.informationDialogTitle = "添加发动机型号";
    ...
    selectedEngineList(engineTypeList) {
      this.engineIds = [];
      engineTypeList.forEach((item,index) => {
        this.engineIds.push(item.id)
      });
      console.log(JSON.stringify(this.engineIds))
    },
    ...
    editRowInfo(form) {
      ...
      this.$refs.informationDialog.form.power = form.power;
      this.$refs.informationDialog.form.brand = form.brand;
      this.$refs.informationDialog.form.firm = form.firm;
      this.$refs.informationDialog.form.oil_type = form.oil_type;
      this.$refs.informationDialog.form.cylinder_count = form.cylinder_count;
      this.$refs.informationDialog.form.remark = form.remark;
      this.informationDialogTitle = "编辑发动机型号";
      this.informationDialogShow = true;
    },
    // 删除
    deleteRowInfo(info) {
      ...
      if(!!info) {
        ...
        params.context = this.engineIds
      }
      this.$client.deleteEngineModel(params)
      .then(res => {
        if (res.head.result == "200") {
          this.loadData();
          this.$message({
