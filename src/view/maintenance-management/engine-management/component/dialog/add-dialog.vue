    ...
    <div>
      <el-tabs v-model="activeName">
        ...
        <el-tab-pane label="安装车辆" name="second">
          <install-info ref="installInfo" :curChangeType="curChangeType" :iState="iState" v-loading="detailLoading"></install-info>
        </el-tab-pane>
        <el-tab-pane label="大修情况" name="third" v-if="curChangeType == 'detail'">
        ...
        <el-tab-pane label="生命周期" name="six" v-if="curChangeType == 'detail'">
          <life-cycle ref="lifecycle" v-loading="detailLoading"></life-cycle>
        </el-tab-pane>
      </el-tabs>
    </div>
...
export default {
  ...
  methods: {
    ...
    setData(val){
      ...
      setTimeout(() => {
        this.$refs['baseInfo'].setData(val.engine_base)
        this.$refs['installInfo'].setData(val.engine_vehicle);
        if(this.curChangeType == 'detail') {
          this.$refs['maintainCondition'].setData(val.engine_maintain)
          this.$refs['scrapCondition'].setData(val.engine_scrape)
          this.$refs['lifecycle'].setData(val.engine_life)
        }
