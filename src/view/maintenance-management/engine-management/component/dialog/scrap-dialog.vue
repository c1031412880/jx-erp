  ...
  <el-dialog :visible="dialogBool" :title="showScrapTitle" width="550px"  :before-close="dialogClose">
    <div class="scrap-dialog">
      ...
      > 
        ...
        <el-form-item label="报废原因:" prop="c_scrap_reason">
          <el-input
            type="textarea"
            :autosize="{ minRows: 4, maxRows: 8}"
            placeholder="请输入报废原因"
            v-model="formData.c_scrap_reason"
          ></el-input>
        </el-form-item>
        <el-form-item label="说明:" prop="c_da_remark">
            ...
            <span style="font-size: 12px;font-weight: 400;font-style: normal;color: rgba(102, 102, 102, 0.5);">只有闲置状态下的发动机才允许报废。报废操作不可逆，请谨慎提交。</span>
        </el-form-item>
      </el-form>
    </div>
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$confirm('确认提交报废, 是否继续?', '警告', {
            ...
            this.$emit("sure-submit-scrap", info)
          }).catch(() => {
            this.$message({
              type: 'info',
              message: '已取消删除'
