  ...
  <div class="maintenance-project-manage-box">
    <div class="maintenance-project-manage">
      <el-card class="box-card left-box">
        ...
        >
          <span style="white-space:nowrap;">故障诊断分类</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    loadOrganizationTree() {
      this.organizationloading = true;
      this.$client.getFaultDiagonosisTree()
      .then(res => {
    ...
    // 加载所属类别
    loadClassifyGroup() {
      this.$client.queryFaultDiagnosis()
      .then(res => {
        console.log(res)
        if(res.head.result == "200") {
    ...
    reqOrgProjectList(isCurDepartment) {
      ...
      this.$client.queryRepairProject(params)
      .then(res => {
        ...
        if (res.head.result == "200") {
          ...
          if(!!this.selectorderlist && this.selectorderlist.length) {
            this.$nextTick(() => {
    ...
    onSubmitExcel(val) {
      ...
      let params = formData;
      this.$client.importRepairProject(params)
      .then(res => {
        if(res.head.result == "200") {
          this.form = {};
          this.pageInfo.pageIndex = 1;
