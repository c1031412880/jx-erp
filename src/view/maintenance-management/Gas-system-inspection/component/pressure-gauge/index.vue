...
export default {
  ...
  data() {
    return {
      ...
      FileData: {
        url: window.newHttpConfig.URL_EASYMOCK +  "/Files/TempFiles/压力表导入模板.xlsx",
      },
  ...
  methods: {
    ...
    loadData() {
      ...
      this.$client.GetPressureGageByPage({context:params})
        .then(res => {
    ...
    addOrEditInformation(form) {
      ...
      this.$client
        .AddOrUpdatePressureGage(parmas)
        .then((res) => {
          if (res.head.result == "200") {
            this.loadData();
    ...
    deleteRowInfo(info) {
      ...
      this.$client.DeletePressureGage(params)
      .then(res => {
        if (res.head.result == "200") {
          this.loadData()
          this.$message({
    ...
    saveInspect(data) {
      this.$client.BatchCheckPressureGage({context:data}).then((res) => {
        if(res.head) {
          if (res.head.result == "200") {
    ...
    onExport() {
      let params = Object.assign({}, this.form);
      this.$client.PressureGageExportGasManage({context:params}).then((res) => {
        if(res.head) {
          if (res.head.result == "200") {
    ...
    importExcel(val){
      ...
      let params = formData;
      // console.log(params)
      this.$client.PressureGageImportGasManage(params)
      .then(res => {
        if(res.head.result == "200") {
          this.loadData()
