      ...
      <el-form :inline="true" size="mini">
        <el-row :gutter="20">
          ...
          <el-form-item label="压力表编号:" prop="gage_code">
            <el-input v-model="form.gage_code" clearable placeholder="请输入"></el-input>
          </el-form-item>
          <el-form-item>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if (this.form.gage_code) {
        data = data.filter(item => {
          return item.gage_code.includes(this.form.gage_code)
        })
      }
      this.data = data
    },
    ...
    onExport() {
      ...
      this.$client.CheckExportPressureGage({context:params}).then((res) => {
        if(res.head) {
    ...
    importExcel(val){
      ...
      let params = formData;
      this.$client.CheckImportPressureGage(params)
      .then(res => {
        if(res.head.result == "200") {
          ...
          this.old_data.forEach(item=> {
            list.forEach(child => {
              if (item.gage_code == child.gage_code) {
