<template>
  <tr-dialog 
    ...
    <div slot="footer" class="dialog-footer" v-if="curChangeType != 'detail' || (curChangeType == 'detail' && curSelectType == 'getCar')">
      ...
      <el-button type="" v-if="curSelectType == 'getCar'" @click="getCarSubmit('退回')">退回</el-button>
      <el-button type="primary" v-if="curSelectType == 'getCar'" @click="getCarSubmit('接车')">确认维修项目并接车</el-button>
    </div>
  </tr-dialog>
</template>
...
export default {
  ...
  props: {
    ...
    curSelectType: {
      type: String,
      default: 'detail', // detail详情  getCar 接车
    },
    showCollapse:{
      type: String,
  ...
  watch: {
    dialogBool(bool) {
      if (bool === false) {
        ...
        let hei = (this.curChangeType != 'detail' || (this.curChangeType == 'detail' && this.curSelectType == 'getCar')) ? 195 : 125
        this.boxHeight = document.body.clientHeight - hei + 'px';
        if(this.curSelectType == 'getCar') this.activeName = 'fourth';
      }
    },
  },
  methods: {
    changeBoxHeight() {
      let hei = (this.curChangeType != 'detail' || (this.curChangeType == 'detail' && this.curSelectType == 'getCar')) ? 195 : 125
      this.boxHeight = this.boxHeight === '70vh' ? document.body.clientHeight - hei + 'px' : '70vh';
    },
    configAddStaffInfo() {
    ...
    setData(val){
      ...
      setTimeout(() => {
        ...
        this.info = this.curSelectType == 'getCar' ? {...val} : {}
      }, 60);
    },
    dialogClose() {
    ...
    getCarSubmit(type) {
      this.$emit('get-car-submit', this.info, type)
    },
  }
};
</script>
