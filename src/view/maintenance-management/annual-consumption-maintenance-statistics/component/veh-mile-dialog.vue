<template>
  ...
  >
    <div slot="title">
      <p style="margin-bottom: 10px;">里程明细</p>
      <p>
        <span>日期范围:</span>
        <span style="color: red">{{ form.begin_time }}至{{ form.end_time }}</span>
      </p>
    </div>
  ...
  methods: {
    ...
    loadData() {
      if(this.pageNum == 1) {
        ...
        this.$client.GetEffectDetailAnnualFuelConsumption(content).then(req => {
          this.loading = false;
          if (req.head.result === "200") {
            ...
            this.sumTotal = { //约定某一列汇总合计自定义数据
              ...
              5: req.context.summary.count,
              6: req.context.summary.stop_mileage,
              7: req.context.summary.transfer_mileage,
              8: req.context.summary.total_mileage,
            }
          } else {
            this.data = [];
        ...
        content.page_index = this.pageInfo.pageIndex;
        content.page_size = this.pageInfo.pageSize;
        this.$client.GetGpsDetailAnnualFuelConsumption(content).then(req => {
          this.loading = false;
          if (req.head.result === "200") {
            this.data = req.context.list;
            this.pageInfo.pageCount = req.context.total;
