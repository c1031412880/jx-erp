  ...
  methods: {
    ...
    // },
      setHead() {
          ...
          this.TableHeight = maxh - (th) - 120 - 0
    ...
    // isCurDepartment //是否是当期组织还是上级组织
      ...
      .then(res => {
        ...
        if (res.head.result == "200") {
          ...
          this.$nextTick(() => {
            const arr = this.data.filter(element => this.selectorderlist.findIndex(e => element.c_code === e.c_code) >= 0)
            // this.$refs.trTabel.toggleSelection()
            this.$refs.trTabel.toggleSelection(arr)
            // this.$refs.trTabel.toggleSelection([this.data[0]])
            // this.setSelectTabel()

          })

          console.log(this.data[0],this.selectorderlist,'6678')
          } else {
            this.loading = false;
            this.$message({
              showClose: true,
    ...
    setSelectTabel(val, type) {
      this.type = type
      console.log(val);
      let info = {
        c_name: val.item_name || null,
        c_code: val.code || null,
        c_remark: val.c_remark || null,
        group_name: val.group_name || null,
        i_group_id: val.i_group_id || null,
        i_hours: val.hours || null,
        i_id: val.item_id || null,
        i_order: val.order_id || null,
        n_price: val.price || null,
      }
      setTimeout(() => {        
        this.$refs.trTabel.onSelectedRowInfo(info)
      }, 300);
      // this.$refs.trTabel.toggleSelection()
      // this.selectorderlist= val
      // const arr = this.data.filter(element => val.findIndex(e => element.c_code === e.c_code) >= 0) // 这段出来就是一个数组【】
      // console.log(this.selectorderlist,[this.data[0]],'667')
      // this.$refs.trTabel.toggleSelection(arr)
