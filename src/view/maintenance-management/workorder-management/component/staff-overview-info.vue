  ...
  <div class="staff-overview-info" style="width: calc(100% - 10px);overflow: hidden;margin: 0 5px">
    ...
    <el-row :gutter="20" type="flex">
      <el-col :xl="4" :lg="8" v-for="(item,index) in data" :key="index" style="height: 100%">
        <div class="show-flex-box-r card-panel" @click="onSubmit(index)">
...
export default {
  ...
  methods: {
      onSetTime(type = 'today') {
      let timeObj = getDate(type)
      this.date = this.$addTime([timeObj.startTime, timeObj.endTime])
      this.form.create_start = this.date[0]
      this.form.create_end = this.date[1]
    },
     onSetTime1(type = 'today') {
      let timeObj = getDate(type)
      this.date = this.$addTime([timeObj.startTime, timeObj.endTime])
      this.form.finish_check_start  = this.date[0]
      this.form.finish_check_end  = this.date[1]
    },
    onSubmit(i){
      if(i == 0){
        this.onSetTime()
    ...
    resetData(){
        this.form.state=''
        this.form.is_state=''
        this.form.finish_check_start = ''
        this.form.finish_check_end =''
