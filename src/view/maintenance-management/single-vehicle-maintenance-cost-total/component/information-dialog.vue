  ...
  >
    <div slot="title">
      <div class="show-flex-box-r" style="align-items: center">
        <h2 style="font-size: 20px; font-weight: 600;margin-right: 80px">
          查看明细
        </h2>
        <div class="show-flex-box-r">
          <strong style="margin-right: 15px">当前车辆:</strong>
          <span style="margin-right: 30px;color:#D9001B">{{
            form.vehicle_name
          }}</span>
          <strong style="margin-right: 15px">日期范围:</strong>
          <span style="color:#D9001B"
            >{{ form.start_date }}至{{ form.end_date.slice(0, 10) }}</span
          >
        </div>
      </div>
    </div>
          ...
          >
            ...
            >
              <el-button
                type="text"
                size="mini"
                @click="jumpOrderPage(scope.data)"
                >{{ scope.data.work_code }}</el-button
              >
            </template>
            <template
              slot-scope="scope"
              slot="state_detail_name"
              v-if="item.value == 'first'"
            >
            ...
            <template
              slot-scope="scope"
              slot="vehicle_kind_name"
              v-if="item.value == 'first'"
            >
              <span v-if="scope.data.vehicle_info == null"></span>
              <span v-else>{{
                scope.data.vehicle_info.vehicle_kind_name
              }}</span>
            </template>
            <template
              slot-scope="scope"
            ...
            >
              <el-button
                type="text"
                @click="onSelectRow(scope.data, scope.data.maint_dept_name)"
                >{{ scope.data.item_name }}</el-button
              >
            </template>
            <template
              slot-scope="scope"
            ...
            >
              <el-button
                type="text"
                @click="onSelectRow(scope.data, scope.data.maint_dept_name)"
                >{{ scope.data.person_name }}</el-button
              >
            </template>
            ...
            <template
              slot-scope="scope"
              slot="leave_num"
              v-if="item.value == 'second'"
            >
              <el-button
                type="text"
                size="mini"
                @click="jumpWarehouseManagement(scope.data)"
                >{{ scope.data.leave_num }}</el-button
              >
            </template>
          </tr-table>
