<template>
  <div v-loading="loading" class="employee-info" id="employee-info"></div>
</template>
...
export default {
  ...
  methods: {
    resizeChart() {
      this.empChart && this.empChart.resize()
    },
    loadData() {
      this.loading = true
      this.$client.GetBusData().then(res => {
        ...
        if (res.head.result == '200') {
          this.initChart(res.context.bus_overview.user_count, res.context.user);
        } else {
          this.$message({
            showClose: true,
    ...
    initChart(total = 0, data = []) { 
      this.empChart = echarts.init(document.getElementById('employee-info'));
      const option = {
        color: [
      ...
      this.empChart.setOption(option);
    },
    beforeDestroy() {
      this.empChart && this.empChart.dispose();
    }
  }
}
</script>
