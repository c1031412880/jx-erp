<template>
  <div v-loading="loading" class="line-info" id="line-info"></div>
</template>
...
export default {
  ...
  methods: {
    resizeChart() {
      this.linChart && this.linChart.resize()
    },
    loadData() {
      this.loading = true
      this.$client.GetBusData().then(res => {
        ...
        if (res.head.result == '200') {
          this.initChart(res.context.bus_overview.line_count, res.context.line);
        } else {
          this.$message({
            showClose: true,
    ...
    initChart(total = 0, data = []) { 
      this.linChart = echarts.init(document.getElementById('line-info'));
      const option = {
        color: [
      ...
      this.linChart.setOption(option);
    },
    beforeDestroy() {
      this.linChart && this.linChart.dispose();
    }
  }
}
</script>
