  ...
  <div class="unconfirmed-separation">
    ...
    <tr-table
      ...
      <template slot-scope="scope" slot="operation">
        <div v-if="!resignated">
          <el-button type="primary" size="mini" v-if="$isPowerShow('confirm')" @click.stop="sureLeave(scope.data)">确认离职</el-button>
          <el-button type="danger" size="mini" @click.stop="cancleLeaveApply(scope.data)">撤销</el-button>
        </div>
        <div v-else>
          <el-button type="text" size="mini" @click.stop="openSelectRowPage(scope.data)">查看</el-button>
        </div>
      </template>
    </tr-table>
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      params.is_deal = this.resignated ? 1 : 0
      params.page_index = this.pageInfo.pageIndex;
      params.page_size = this.pageInfo.pageSize;
      this.$client.listDriverLeave(params)
        .then(res => {
    ...
    exportFile() {
      ...
      this.$client.exportDriverLeave(params)
      .then(res => {
        if(res.head.result == '200') {
    ...
    checkInfo(info) {
      ...
      if(info.source === 1) {
        ...
        // 打开审批单
        let detail_id = 1470, title = '代办事项:驾驶员离职退休申请'
        this.$router.push({
          path: `/news-details/${info.leave_id}/${title}/${detail_id}`
        })
      }
    },
    // 确认离职
    ...
    cancleLeaveApply(form) {
      this.$confirm('是否确认撤销离职，确认后将不可撤回', '提示', {
          ...
          this.$client.cancelDriverLeave(parmas).then((res) => {
            if (res.head.result == "200") {
              this.loadData();
              this.$message({
                showClose: true,
