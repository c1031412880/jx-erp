  ...
  <div ref="organizationTree" class="organization-tree">
    ...
    >
      ...
      <span class="custom-tree-node" slot-scope="{ node, data }">
        <span>
          <i style="margin-right: 5px; color:#409EFF" :class="'el-icon-office-building'"></i
          >{{ node.data.score == 0 ? node.data.platform : `${node.data.platform}(${node.data.score})` }}
        </span>
      </span>
...
export default {
  ...
  methods: {
    filterNode(value, data) {
      ...
      return data.platform.indexOf(value) !== -1;
    },
