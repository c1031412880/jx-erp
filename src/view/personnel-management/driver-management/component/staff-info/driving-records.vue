...
<!-- 
      ...
      <div class="el-form-row" v-if="false">
        <el-form-item label="初始化总里程（KM）:" prop="init_total_mile">
          <el-input
            oninput="value=value.replace(/[^\d.-]/g,'')"
            @blur="formData.init_total_mile = $event.target.value"
        ...
        <el-form-item label="公交总里程（KM）:" prop="bus_total_mile">
          <el-input
            oninput="value=value.replace(/[^\d.-]/g,'')"
            @blur="formData.bus_total_mile = $event.target.value"
      ...
      <div class="el-form-row" v-if="false">
        <el-form-item label="当前安全里程（KM）:" prop="safe_mile">
          <el-input
            oninput="value=value.replace(/[^\d.-]/g,'')"
            @blur="formData.safe_mile = $event.target.value"
        ...
        <el-form-item label="初始安全里程（KM）:" prop="init_safe_mile">
          <el-input
            oninput="value=value.replace(/[^\d.-]/g,'')"
            @blur="formData.init_safe_mile = $event.target.value"
...
export default {
  ...
  data() {
    return {
      ...
      formData: {
        ...
        d_latest_get_licence: '', //驾驶证到期日期
        d_qualification_end: '', //从业资格证到期日
      },
      rules: {
  ...
  methods: {
    ...
    setData(val){
      ...
      this.formData.d_latest_get_licence = val.d_latest_get_licence
      this.formData.d_qualification_end = val.d_qualification_end
