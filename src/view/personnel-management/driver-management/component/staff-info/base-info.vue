    ...
    >
      <div class="el-form-row-first">
        <div class="el-form-row-first-left">
          ...
          <el-form-item label="副部门:" prop="sub_orgs">
            <get-department-select
              ref="departmentTree"
              :isContainEmp="isContainEmp"
          ...
          <!-- <el-form-item label="岗位:" prop="position_id">
            <get-position-list v-model="formData.position_id" :disabled="curChangeType == 'detail'" v-if="showCollapse == 'first'"></get-position-list>
            <el-input
              placeholder="请选择岗位"
      ...
      <div class="el-form-row" v-if="showCollapse == 'second'">
        <el-form-item label="线路:" prop="line_names">
            <el-input
              placeholder="请输入线路"
              v-model="formData.line_names"
              disabled
            ></el-input>
        </el-form-item>
        <el-form-item label="车辆:" prop="vehicle_names">
            <el-input
              placeholder="请输入车辆"
              v-model="formData.vehicle_names"
              disabled
            ></el-input>
        </el-form-item>
      </div>
  ...
  methods: {
      ...
      this.$client.getSysPositionList(parmas)
        .then(res => {
          if (res.head.result == "200") {
            this.changePosition(res.context.list)
          } else {
            this.$message({
              showClose: true,
    ...
    checkIdCardIsUses(val) {
      ...
      if(checkCard(val)){
        ...
        console.log('this.formData.d_birthday',this.formData.d_birthday);
        this.selectedBirthday(this.formData.d_birthday)
      }
    },
    // 设置驾驶证号
    ...
    setData(val){
      ...
      // this.formData.line_names = val.line_names
      // this.formData.vehicle_names = val.vehicle_names
    },
    // 给父级页面提供得获取本页数据
    getData() {
