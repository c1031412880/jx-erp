  ...
  >
    <div slot="title">
      <div class="show-flex-box-r" style="height: 50px; background: #f5f5f5f5; padding-left: 15px;">
        <em style="line-height: 50px;" v-if="!!form.date">{{form.date}} {{week}}</em>
      </div>
    </div>
    <div class="el-dialog-div">
      ...
      >
        <template slot-scope="scope" slot="state_name">
          <span :style="`${scope.data.state != 1? 'color:#D9001B;cursor: pointer;':''}`" @click="updateKqInfo(scope.data)">{{showText(scope.data)}}</span>
        </template>
            ...
            <el-form-item label="" prop="">
              <div class="show-flex-box-r" style="align-items:center">
                <el-radio-group v-model="updateForm.state">
                  ...
                  <el-radio :label="5" v-if="curSelectInfo.type == 1 && (curSelectInfo.state == 2 || curSelectInfo.state == 3)">缺卡</el-radio>
                  <el-radio :label="2"  v-if="curSelectInfo.type == 1 && (curSelectInfo.state == 5 || curSelectInfo.state == 2 || curSelectInfo.state == 6)">迟到</el-radio>
                  <el-radio :label="3" v-if="curSelectInfo.type == 2 && (curSelectInfo.state == 3)">早退</el-radio>
...
export default {
  ...
  methods: {
    ...
    setDateInfo() {
      if(!!this.form.date) {
        this.week =  this.getweekday(this.form.date)
      }
      // 获取详情
      this.reqKqRequest()
    ...
    sureUpdate() {
      ...
      if(this.updateForm.state == 3  && !this.updateForm.minutes) {
       ...
       this.$client
          .UpdateClockKqRecord(this.updateForm)
          .then((res) => {
            if (res.head.result == "200") {
              this.reqKqRequest();
              this.curSelectInfo = {};
