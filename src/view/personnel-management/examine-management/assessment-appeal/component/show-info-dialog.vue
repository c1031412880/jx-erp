        ...
        <el-row :gutter="20">
          ...
          <el-col :span="24" class="table-box">
            ...
            <tr-table
              ...
              <template slot-scope="scope" slot="last_score">
                <div v-if="curSelecedType == 'edit' && scope.data.score_person == userId && (scope.data.appeal_reason || scope.data.appeal_files.length > 0)">
                  <el-input 
                  v-model="scope.data.last_score" 
                  clearable 
                  @change="changeScore(scope.data,scope.index)"
                  oninput="value=value.replace(/[^\d.-]/g,'')" 
                  @blur="scope.data.last_score = $event.target.value"
                  ></el-input>
                </div>
                <div v-else>
                  {{scope.data.last_score}}
                </div>
              </template>
              <template slot-scope="scope" slot="deal_reason">
                <div v-if="curSelecedType == 'edit' && scope.data.score_person == userId && (scope.data.appeal_reason || scope.data.appeal_files.length > 0)">
                  <el-input v-model="scope.data.deal_reason" clearable></el-input>
                </div>
                <div v-else>
                  {{scope.data.deal_reason}}
                </div>
              </template>
              <template slot-scope="scope" slot="deal_files">
                <el-upload
                  v-if="curSelecedType == 'edit' && scope.data.score_person == userId && (scope.data.appeal_reason || scope.data.appeal_files.length > 0)"
                  class="upload-demo"
                  ref="uploadDocomnet"
                  action="string"
                ...
                <div v-for="(item,index) in scope.data.deal_files" :key="index">
                  <div style="display:flex;align-items:center">
                    <el-tooltip class="item" effect="dark" :content="item.name" placement="top">
                      <div @click="downloadDocument(item)" style="color:#409EFF;flex:1;cursor: pointer;">{{item.name}}</div>
                    </el-tooltip>
                    <i class="el-icon-delete" v-if="curSelecedType == 'edit' && scope.data.score_person == userId && (scope.data.appeal_reason || scope.data.appeal_files.length > 0)" style="color:red;width:50px;cursor: pointer;" @click="deleteDocument(scope.index,index)"></i>
                  </div>
                </div>
              </template>
            </tr-table>
      ...
      .then(res => {
        if (res.head.result == "200") {
            ...
            this.tableList.forEach(item => {
              item.last_score = item.last_score === null ? '' : item.last_score
            })
