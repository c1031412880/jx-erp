  ...
  <div class="assessment-appeal">
    ...
    > 
      ...
      <template slot-scope="scope" slot="operation">
        <el-button type="text" size="mini" style="margin-left: 10px;" @click="showDetails(scope.data,'detail')">考核详情</el-button>
        <el-button type="text" size="mini" style="margin-left: 10px;" v-if="scope.data.appeal_state == 1 && scope.data.exam_checks.indexOf(userId) != - 1" @click="showDetails(scope.data,'edit')">处理</el-button>
        <el-button type="text" size="mini" style="margin-left: 10px;" v-if="scope.data.appeal_state == 2 && scope.data.user_id == userId" @click="showDetails(scope.data,'check')">审核</el-button>
      </template>
    </tr-table>
  ...
  methods: {
    ...
    saveAppeal(params) {
      if (this.curSelecedType == 'edit') {
        this.$client.DealAssessAppeal({context:params})
          .then(res => {
            if (res.head.result == "200") {
        ...
        this.$client.AuditAssessAppeal({context:params.id})
        .then(res => {
            if (res.head.result == "200") {
              this.isShowSetInfoDialog = false
              // 重新加载列表详情
