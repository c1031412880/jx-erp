      ...
      <div>
        ...
        >
          ...
          <template slot-scope="scope" slot="operation">
            ...
            <div v-if="scope.data.push_state == 0" style="display:inline-block">
              <el-popconfirm
                @confirm="releaseRowInfo(scope.data)"
                title="确认后将无法撤回，是否确认发布考核?">
                <el-button slot="reference" type="text" size="mini" style="margin-left: 10px;color:red">发布</el-button>
              </el-popconfirm>
            </div>
          </template>
        </tr-table>
...
export default {
  ...
  methods: {
    ...
    // 发布
    releaseRowInfo(info){
      this.$client.ReleaseAssessRecord({context:info.id})
      .then(res => {
        if (res.head.result == "200") {
          // 重新加载列表详情
          this.loadData();
