        ...
        <el-row :gutter="20">
          ...
          <el-col :span="24" class="table-box">
            ...
            <tr-table
              ...
              <template slot-scope="scope" slot="appeal_reason">
                <div v-if="curSelecedType == 'detail'">
                  {{scope.data.appeal_reason}}
                </div>
                <div v-else>
                  <el-input v-model="scope.data.appeal_reason" clearable></el-input>
                </div>
              </template>
              <template slot-scope="scope" slot="appeal_files">
                ...
                <div v-for="(item,index) in scope.data.appeal_files" :key="index">
                  <div style="display:flex;align-items:center">
                    <el-tooltip class="item" effect="dark" :content="item.name" placement="top">
                      <div @click="downloadDocument(item)" style="color:#409EFF;flex:1;cursor: pointer;">{{item.name}}</div>
                    </el-tooltip>
  ...
  methods: {
    ...
    updatePlaybill() {
      ...
      this.tableList.forEach(item => {
        if (item.appeal_reason || item.appeal_files.length > 0) {
          let obj = {
            content_id: item.id,
            old_score: item.exam_score,
            appeal_reason: item.appeal_reason || '',
            file_ids: item.appeal_files.map(child => {return child.id}),
          }
          list.push(obj)
        }
      })
    ...
    handlePreview(file,index) {
      ...
      this.$client.NewAddTxtfileUpload(params).then(res => {
        if(res.head.result == "200") {
          ...
          this.tableList[index].appeal_files.push(fileObj)
          this.$message({
            showClose: true,
            message: "上传成功",
            type: "success"
    ...
    deleteDocument(rowIndex,index) {
      this.tableList[rowIndex].appeal_files.splice(index, 1)
    },
  }
};
</script>
