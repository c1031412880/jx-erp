  ...
  <el-dialog :visible="dialogBool" :title="showTitle" :before-close="dialogClose" width="900px"  :append-to-body="appendToBody">
    <div>
      <el-tabs v-model="activeName">
        ...
        <el-tab-pane label="奖惩记录" name="seventh" v-if="curChangeType == 'detail'">
          <reward-punishment-records ref="rewardPunishmentRecords"></reward-punishment-records>
        </el-tab-pane>
        <el-tab-pane label="变更记录" name="eighth" v-if="curChangeType == 'detail'">
          <change-record ref="changeRecord"></change-record>
        </el-tab-pane>
        <el-tab-pane label="成长记录" name="ninth" v-if="curChangeType == 'detail'">
    ...
    <div slot="footer" class="dialog-footer" v-if="curChangeType != 'detail'">
      ...
      <el-button v-loading="btnLoading" type="primary" @click="submitFormData">确 定</el-button>
    </div>
  </el-dialog>
</template>
  ...
  methods: {
    ...
    changePersonalName(name,type) {
      ...
      if(type == 'base') {
        ...
        this.$refs.userInfo.formData.c_login_id = name
      }else if(type == 'user'){
