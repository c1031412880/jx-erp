  ...
  <div class="staff-overview-info" style="width: calc(100% - 10px);overflow: hidden;margin: 0 5px">
    ...
    <el-row :gutter="20" type="flex">
      <el-col :xl="4" :lg="8" v-for="(item,index) in data" :key="index" style="height: 100%">
        <div class="show-flex-box-r card-panel" @click="jumpNewsPages(item)">
...
<script>
import p1 from "../../../../assets/staff-manage-image/1.png";
import p2 from "../../../../assets/staff-manage-image/2.png";
import p3 from "../../../../assets/staff-manage-image/3.png";
import p4 from "../../../../assets/staff-manage-image/4.png";
import p5 from "../../../../assets/staff-manage-image/5.png";
...
export default {
  ...
  methods: {
    ...
    getEmploymentForm() {
      ...
      params.context.push('employment-form')
      this.$client.getDictionaryDataBase(params).then(res => {
        if (res.head.result === '200') {
          if(res.context && res.context.length) {
            this.dictionaryList = res.context[0].dic_details ? res.context[0].dic_details : []
          }
        }
      })
    },
    ...
    configStaffCount(staffTypeCountList) {
      // alert(111)
      staffTypeCountList.forEach((item,index) => {
        this.data.forEach((zItem,zIndex) => {
          if(item.type == zItem.labelTxt) {
            zItem.numberCount = item.count
          }
        });
      });
    },
    ...
    jumpNewsPages(item) {
      ...
      switch (item.type) {
        ...
        case 'sixth':
          ...
          break;
        case 'fourth':
          let data = this.dictionaryList.find(item => {
            return item.c_name == '正式编制'
          })
          from.c_employee_type = data.i_id ? data.i_id : ''
          break;
        case 'fifth':
          let list = this.dictionaryList.find(item => {
            return item.c_name == '其他'
          })
          from.c_employee_type = 0
          break;
      ...
      if(!!from.c_employee_type || from.c_employee_type == 0) {
        this.$emit("on-ok", from);
      }
      if(isReset) {
         this.$emit("on-reset");
      }
