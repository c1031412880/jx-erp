<template>
  ...
  >
    <div class="el-dialog-div">
      <el-form ref="form" size="mini" :model="form" :rules="rules" label-width="150px" label-suffix=":">
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="关联审批单" prop="rule_name">
              <!-- <el-input v-model="form.rule_name" style="width:100%" disabled placeholder="请选择关联维修单"></el-input> -->
              <el-input v-model="repair_name" @click.native="choiceOver()" class="over" style="width:100%"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="加油日期" prop="oil_date">
              <el-date-picker
                :disabled="disabled"
                style="width:100%"
          ...
          <el-col :span="12">
            <el-form-item label="加油时公里数(公里)" prop="oil_mile">
              <el-input-number v-model="form.oil_mile" style="width:100%" :disabled="disabled"  :min="0" :precision="2" :controls="false"></el-input-number>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="加油类型:" prop="type">
              <tr-select-dictionaries v-model="form.type" :disabled="disabled" style="width:100%" :classKey="'公车加油类型'" :placeholder="'请选择加油类型'" :isMultiple="false"></tr-select-dictionaries>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="加油金额（元）" prop="oil_fee">
              <el-input-number v-model="form.oil_fee" style="width:100%" :disabled="disabled"  :min="0" :precision="2" :controls="false"></el-input-number>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="加油负责人" prop="oil_user">
              <get-department-select
                ref="personalTree"
                v-model="form.oil_user"
    ...
    <el-dialog
      ...
      <el-table
        ...
        <el-table-column label="加油日期" width="120" prop="oil_date"></el-table-column>
        <el-table-column label="车牌号" width="120" prop="code"></el-table-column>
        <el-table-column label="加油时公里数（公里）" width="200" prop="oil_mile"></el-table-column>
        <el-table-column label="加油类型" prop="type_name"></el-table-column>
        <el-table-column label="加油金额（元）" width="170" prop="oil_fee"></el-table-column>
        <el-table-column label="加油位置" width="150" prop="station_postion"></el-table-column>
        <el-table-column label="审批单编号" width="160">
          <template slot-scope="scope" >
            <el-button type="text" @click="openApply(scope.row, 1)">{{scope.row.flow_code}}</el-button>
          </template>
        </el-table-column>
        <el-table-column label="审批状态" width="120" prop="state_name"></el-table-column>
      </el-table>
...
export default {
  ...
  mounted() {
    this.form.oil_date = formatDate(new Date(),"yyyy-MM-dd")
    this.form.bus_code = this.bus_code
    this.form.oil_mile = this.last_mile
    if (getUser()) {
      this.form.oil_user = Number(JSON.parse(getUser()).user_id)
    }
    this.GetListOaFormOilReq()
  },
  methods: {
    choiceOver() {
    ...
    checked_name() {
      ...
      this.repair_name = this.overInfo.user_name + "提交的公车加油单"
      this.form.oil_fee = this.overInfo.oil_fee
      this.form.type = this.overInfo.oil_type
      this.overVisible = false
    },
    // 多选只能选一个
    ...
    GetListOaFormOilReq() {
      ...
      this.$client.GetListOaFormOilReq(params).then(res => {
    ...
    openApply(row, type) {
      ...
      // let detail_id = 1220, title = '公车加油申请'
      // this.$router.push({
