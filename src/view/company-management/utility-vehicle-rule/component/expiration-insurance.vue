<template>
  <div class="expiration-insurance">
    <div ref="header" style="display:flex;align-items:center;margin-bottom:15px">
      ...
      <p style="margin-left:25px;color:#000;font-size:16px;">保险到期提醒</p>
    ...
    <insurance-rule-dialog
      ref="insuranceRuleDialog"
...
import insuranceRuleDialog from '../rule-dialog/insurance-rule-dialog'
export default {
  name: "expiration-insurance",
  components: {insuranceRuleDialog},
  props: {
    TableHeight: [Number],
  },
  data() {
  ...
  methods: {
    // 删除
    deleteRowInfo(row) {
      this.$client.DeleteBusMsgInsurance({context:[row.id]}).then(res =>{
    ...
    editRowInfo(row) {
      ...
      this.ruleDialog = true
      setTimeout(() => {
        this.$refs['insuranceRuleDialog'].setData(row)
      }, 200);
    },
    // 启用、禁用
    changeSwitch(id,enable) {
      ...
      this.$client.EnableBusMsgInsurance({context:params}).then(res =>{
        if (res.head.result == "200") {
            this.loadData()
            this.$message({
              showClose: true,
    ...
    submit(form,type) {
      if (type == "add") {
        this.$client.CreateBusMsgInsurance({context:form}).then(res =>{
          console.log('res',res);
          if (res.head.result == "200") {
        ...
        this.$client.UpdateBusMsgInsurance({context:form}).then(res =>{
          console.log('res',res);
          if (res.head.result == "200") {
            this.ruleDialog = false
            this.loadData()
    ...
    loadData() {
      this.loading = true;
      this.$client.GetBusMsgInsuranceList()
        .then(res => {
          if (res.head.result == "200") {
            this.loading = false;
