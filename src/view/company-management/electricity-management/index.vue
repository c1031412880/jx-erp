<template>
  <div class="electricity-management-box">
    <div class="electricity-management">
      <el-card class="box-card left-box">
        ...
        >
          <span style="white-space:nowrap;">电表选择</span>
        </div>
          ...
          <div class="show-flex-box-r" style="margin-bottom:15px;" v-if="!!curSelectedDepartment">
            ...
            <el-popover
              ...
              <div class="show-flex-box-r" style="">
                ...
                <div class="show-flex-box-c" >
                  <p style="font-size: 18px; margin-bottom: 5px;">{{`${curSelectedDepartment.type === 2?`该电表下有${data.length}个记录`:`该地址下有${ curSelectedDepartment.children && curSelectedDepartment.children.length}个电表`}`}}。确认删除吗</p>
...
export default {
  ...
  data() {
    return {
      ...
      FileData: {
        url: `${window.newHttpConfig.URL_EASYMOCK}/Files/TempFiles/电费记录模板.xlsx`,
      },
      form:{}, //查询表单数据
      isSort: false, //是否排序
      tableConfig: {
  ...
  methods: {
    ...
    selectDepartment(departmentInfo) {
      ...
      setTimeout(() => {
        ...
        if(departmentInfo.type === 2) {
          ...
          this.$refs.setOrganizationInfo.ruleForm.electric_id = departmentInfo.id;
        }
        // 查询
        this.$refs.head.onSubmit();
      },50)
    ...
    reqOrgProjectList(isCurDepartment) {
      ...
      this.$client.electricListWaterAndElectricFee(params)
      .then(res => {
      ...
      switch (type) {
        case "add":
          ...
          if(this.curSelectedDepartment && this.curSelectedDepartment.type === 2) {
            ...
            this.$refs.setOrganizationInfo.ruleForm.electric_id = this.curSelectedDepartment.id;
          }
    ...
    editRowInfo(info) {
      ...
      if(!!info) {
        ...
        setTimeout(() => {
          ...
          this.$refs.setOrganizationInfo.ruleForm.parents_id = newForm.ex_elctric_belong_id;
    ...
    saveAddEdit(form) {
      ...
      this.$client
        .addElectricWaterAndElectricFee(params)
        .then((res) => {
          if (res.head.result == "200") {
            this.reqOrgProjectList(true);
            // this.loadOrganizationTree();
    ...
    exportExcel(){
      ...
      this.$client.exportElectricWaterAndElectricFee(params)
      .then(res => {
        if(res.head.result == '200') {
    ...
    suerDeleteClassifyGroup() {
      ...
      this.$client
        .batchElectricGroupDelete(params)
        .then((res) => {
          if (res.head.result == "200") {
            // 排序成功重新加载组织树
            this.loadOrganizationTree();
    ...
    deleteRowInfo(info) {
      ...
      this.$client.batchElectricRecordDelete(params)
      .then(res => {
        if (res.head.result == "200") {
          // if(!this.curSelectedDepartment.type) {
          //   this.pageInfo.pageIndex = 1;
...
.electricity-management /deep/ .el-card__header {
