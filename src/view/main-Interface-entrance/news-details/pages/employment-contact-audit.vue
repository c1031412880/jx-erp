<template>
  <div class="employee-assessment-audit" v-loading="loading">
    ...
    <item-box-new title="基本信息">
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <p>职位</p>
          <em>{{formData.position_name}}</em>
        </el-col>
        <el-col :span="12">
          <p>工号</p>
          <em>{{formData.work_code}}</em>
        </el-col>
        <el-col :span="12">
          <p>驾驶证号</p>
          <em>{{formData.license_code}}</em>
        </el-col>
        <el-col :span="12">
          <p>驾驶证等级</p>
          <em>{{formData.driver_level}}</em>
        </el-col>
        <el-col :span="24">
          <p>备注</p>
          <el-input autosize type="textarea" v-model="formData.remark" placeholder="请输入备注"></el-input>
        </el-col>
        <el-col :span="24" v-if="!(flowFields.opinion === 3)">
          <p>分配意见</p>
          <em v-if="flowFields.opinion === 1">{{`自${formData.opinion_name}安排工作，待上岗后报备公司党群人事部报备。`}}</em>
          <div style="width:100%;display:flex;align-items: center;" v-if="flowFields.post_date === 2">
            <span>自</span>
        ...
        <el-col :span="12" v-if="!(flowFields.post_date === 3)">
          <p>上岗时间</p>
          <em v-if="flowFields.post_date === 1">{{formData.post_date}}</em>
          <el-date-picker
            v-if="flowFields.post_date === 2"
  ...
  methods: {
    ...
    changeInputContent() {
      if(this.formData.state) {
        ...
        this.$client.EditWorkLaborContract(params).then((req) => {})
      }
    },
  }
}
