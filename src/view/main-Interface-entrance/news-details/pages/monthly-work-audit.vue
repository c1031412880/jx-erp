  ...
  <div class="monthly-work-audit" v-loading="loading">
    ...
    <item-box-new title="基本信息">
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <p>当前月份</p>
          <em>{{formData.month}}</em>
        </el-col>
        <el-col :span="24" v-if="!(flowFields.next_plan === 3)">
          <p>下月工作计划</p>
          <el-input autosize type="textarea" class="richText" disabled v-model="formData.next_plan" v-if="flowFields.next_plan === 1"></el-input>
          <el-input autosize type="textarea" v-model="formData.next_plan" placeholder="请输入申请内容" v-if="flowFields.next_plan === 2" @change="changeInputContent"></el-input>
        </el-col>
      </el-row>
  ...
  methods: {
    ...
    changeInputContent() {
      if(this.formData.state) {
        ...
        this.$client.UpdateOaFormNews(params).then((req) => {})
      }
    },
  }
}
