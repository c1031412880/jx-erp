  ...
  <div class="resignation-dispatch-form" v-loading="loading">
    ...
    <item-box-new title="基本信息">
      <el-row :gutter="20">
        ...
        <el-col :span="12" v-if="!(flowFields.open_unit === 3)">
          <p>申请开户单位</p>
          <em v-if="flowFields.open_unit === 1">{{formData.open_unit}}</em>
          <el-input clearable v-model="formData.open_unit" v-if="flowFields.open_unit === 2" @change="changeInputContent"></el-input>
        </el-col>
        <el-col :span="12">
          <p>是否业务要求的开户</p>
          <em>{{formData.is_business == 1 ? '是' : '否'}}</em>
        </el-col>
        <el-col :span="12">
          <p>申请事项</p>
          <em>{{formData.matter_name}}</em>
        </el-col>
        <el-col :span="12">
          <p>是否公开招标</p>
          <em>{{formData.is_public == 1 ? '是' : '否'}}</em>
        </el-col>
        <el-col :span="12">
          <p>项目编号</p>
          <em>{{formData.item_code}}</em>
        </el-col>
        <el-col :span="12">
          <p>是否集团决策</p>
          <em>{{formData.is_group == 1 ? '是' : '否'}}</em>
        </el-col>
        <el-col :span="24">
          <p>会议纪要</p>
          <image-file-upload
          :files="formData.summary_files_list"
          :isShowOriginal="true"
          :isCompress="true"
          :showTitle="''"
          :isShowImages="false"
        ...
        <el-col :span="12">
          <p>开户银行</p>
          <em>{{formData.open_bank}}</em>
        </el-col>
        <el-col :span="24">
          <p>开户原因</p>
          <el-input v-model="formData.open_reason" autosize type="textarea" class="richText" disabled ></el-input>
        </el-col>
      </el-row>
  ...
  methods: {
    ...
    changeInputContent() {
      if(this.formData.state) {
        ...
        this.$client.UpdateBankOpen(params).then((req) => {})
      }
    },
  }
}
