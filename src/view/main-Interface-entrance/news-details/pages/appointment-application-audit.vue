<template>
  <div class="appointment-application-audit" v-loading="loading">
    ...
    <item-box-new title="基本信息">
      <el-row :gutter="20">
        ...
        <el-col :span="24">
          <p>持证情况</p>
          <el-input autosize type="textarea" v-model="formData.hold_prove" placeholder="请输入持证情况"></el-input>
        </el-col>
        <el-col :span="24" v-if="!(flowFields.year === 3)">
          <p>分配意见</p>
          <em v-if="flowFields.year === 1">{{`自${formData.year || 0}年${formData.month || 0}月起可享受每月${formData.fee}元津贴`}}</em>
          <div style="width:100%;display:flex;justify-content: center;align-items: center;" v-if="flowFields.year === 2">
            <span>自</span>
            <el-date-picker
            ...
            <span>年</span>
            <el-select v-model="formData.month" @change="changeInputContent" placeholder="请选择月份">
              <el-option
            ...
            <span style="width: 140px;text-align: center;">月起可享受每月</span>
            <el-input-number v-model="formData.fee" :controls="false" @change="changeInputContent"></el-input-number>
            <span style="width: 80px;text-align: center;">元津贴</span>
          </div>
        </el-col>
      </el-row>
...
export default {
  name: "appointment-application-audit",
  components: {
    itemBoxNew,
    titleBoxNew,
    imageFileUpload,
  ...
  methods: {
    ...
    changeInputContent() {
      if(this.formData.state) {
        ...
        this.$client.UpdatePostAppointment(params).then((req) => {})
      }
    },
  }
}
