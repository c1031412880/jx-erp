  ...
  <div class="resignation-dispatch-form" v-loading="loading">
    ...
    <item-box-new title="基本信息">
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <p>辞职人员姓名</p>
          <em>{{formData.depart_user_name}}</em>
        </el-col>
        <el-col :span="12">
          <p>部门</p>
          <em>{{formData.depart_dept_name}}</em>
        </el-col>
        <el-col :span="12">
          <p>职位</p>
        ...
        <el-col :span="12" v-if="!(flowFields.depart_date === 3)">
          <p>离职日期</p>
          <em v-if="flowFields.depart_date === 1">{{formData.depart_date}}</em>
          <el-date-picker
            v-if="flowFields.depart_date === 2"
  ...
  methods: {
    ...
    changeInputContent() {
      if(this.formData.state) {
        ...
        this.$client.UpdateLaborDepart(params).then((req) => {})
      }
    },
  }
}
