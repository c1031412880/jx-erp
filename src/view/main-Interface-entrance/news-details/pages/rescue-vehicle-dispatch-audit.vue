...
<template>
  <div class="rescue-vehicle-dispatch-audit" v-loading="loading">
    ...
    <item-box-new title="基本信息">
      <el-row :gutter="20">
        ...
        <el-col :span="12" v-if="flowFields.request_date !== 3">
          <p>出车时间</p>
          <em v-if="flowFields.request_date === 1">{{formData.request_date}}</em>
          <el-date-picker
            ...
            v-if="flowFields.request_date === 2"
        ...
        <el-col :span="12">
          <p>支付方式</p>
          <em>{{formData.pay_type_show}}</em>
        </el-col>
        <el-col :span="12">
          <p>施救车型</p>
          <em>{{formData.rescue_vehicle_type_show}}</em>
        </el-col>
        <el-col :span="12" v-if="flowFields.rescue_position !== 3">
          <p>施救地点</p>
          <em v-if="flowFields.rescue_position === 1">{{formData.rescue_position}}</em>
          <el-input v-model="formData.rescue_position" @change="changeInputContent" v-if="flowFields.rescue_position === 2" placeholder="请输入施救地点"></el-input>
        </el-col>
        <el-col :span="24">
          <p>备注</p>
          <el-input v-model="formData.remark" autosize type="textarea" class="richText" disabled ></el-input>
        </el-col>
        <el-col :span="12" v-if="flowFields.arrived_time !== 3">
          <p>到厂时间</p>
          <em v-if="flowFields.arrived_time === 1">{{formData.arrived_time}}</em>
          <el-date-picker
            format="yyyy-MM-dd HH:mm"
            value-format="yyyy-MM-dd HH:mm"
            type="datetime"
            ...
            v-if="flowFields.arrived_time === 2"
            @change="changeInputContent"
            style="width: 100%;">
          </el-date-picker>
        </el-col>
        <el-col :span="12" v-if="flowFields.rescue_amount !== 3">
          <p>施救费用（元）</p>
          <em v-if="flowFields.rescue_amount === 1">{{formData.rescue_amount}}</em>
          <el-input-number v-model="formData.rescue_amount" @change="changeInputContent" v-if="flowFields.rescue_amount === 2"  style="width: 100%;" :min="0" :precision="2" :controls="false"></el-input-number>
        </el-col>
      </el-row>
...
export default {
  name: "rescue-vehicle-dispatch-audit",
  components: {
    itemBoxNew,
    titleBoxNew,
    imageFileUpload,
  ...
  methods: {
    ...
    changeInputContent() {
        ...
        this.$client.EditRescueVehicle(params).then((req) => {})
    },
  }
}
</script>
