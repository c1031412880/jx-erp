...
<template>
  <div class="auto-parts-removed-audit" v-loading="loading">
    ...
    <item-box-new title="基本信息">
      <el-row :gutter="20">
        ...
        <el-col :span="12" v-show="formData.type == 1">
          <p>车牌号（自编号）</p>
          <em>{{formData.vehicle_name_code}}</em>
        </el-col>
        <!-- <el-col :span="12" v-show="formData.type == 1">
          <p>车辆部门</p>
        ...
        <el-col :span="12" v-if="formData.type == 2">
          <p>领用部门</p>
          <em>{{formData.receive_dept}}</em>
        </el-col>
        <el-col :span="12">
          <p>配件名称</p>
          <em>{{formData.part_name}}</em>
        </el-col>
        <el-col :span="12">
          <p>单价（元）</p>
          <em>{{formData.price}}</em>
        </el-col>
        <el-col :span="12">
          <p>数量</p>
          <em>{{formData.count}}</em>
        </el-col>
        <el-col :span="12">
          <p>总价（元）</p>
          <em>{{formData.total_fee}}</em>
        </el-col>
        <el-col :span="24">
          <p>备注</p>
  ...
  methods: {
    ...
    changeInputContent() {
      if(this.formData.state) {
        ...
        this.$client.EditOaVehiclePartsReceive(params).then((req) => {})
      }
    },
  }
}
