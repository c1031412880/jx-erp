<template>
  <div class="corrective-feedback-audit" v-loading="loading">
    ...
    <item-box-new title="基本信息">
      ...
      <el-row>
        <el-col :span="24" v-if="flowFields.repeat_result !== 3">
          <p>复核结果</p>
          <el-input v-if="flowFields.repeat_result === 1" autosize type="textarea" class="richText" disabled v-model="formData.repeat_result"></el-input>
          <el-input type="textarea" v-if="flowFields.repeat_result === 2" v-model="formData.repeat_result" :rows="4" placeholder="请输入复核结果" @change="changeInputContent"></el-input>
        </el-col>
      </el-row>
    </item-box-new>
  </div>
...
export default {
  ...
  methods: {
    ...
    changeInputContent() {
      ...
      this.$client.UpdateHiddenDangerFeedBack(params).then((req) => {})
    },
  }
}
</script>
