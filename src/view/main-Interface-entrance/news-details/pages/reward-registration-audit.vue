  ...
  <div class="resignation-dispatch-form" v-loading="loading">
    ...
    <item-box-new title="基本信息">
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <p>获奖人姓名</p>
          <em>{{formData.reward_name}}</em>
        </el-col>
        <el-col :span="12">
          <p>获奖人所属部门</p>
          <em>{{formData.reward_dept_name}}</em>
        </el-col>
        <el-col :span="12">
          <p>职位</p>
          <em>{{formData.reward_position}}</em>
        </el-col>
        <el-col :span="12">
          <p>身份证号</p>
          <em>{{formData.user_card}}</em>
        </el-col>
        <el-col :span="12" v-if="!(flowFields.reward_fee === 3)">
          <p>建议奖励额度（元）</p>
          <em v-if="flowFields.reward_fee === 1">{{formData.reward_fee}}</em>
          <el-input clearable v-model="formData.reward_fee" v-if="flowFields.reward_fee === 2" @change="changeInputContent"></el-input>
        </el-col>
        <el-col :span="24">
          <p>相关事迹</p>
          <el-input v-model="formData.content" autosize type="textarea" class="richText" disabled ></el-input>
        </el-col>
      </el-row>
  ...
  methods: {
    ...
    changeInputContent() {
      if(this.formData.state) {
        ...
        this.$client.EditRewardRegistration(params).then((req) => {})
      }
    },
  }
}
