  ...
  <div class="resignation-dispatch-form" v-loading="loading">
    ...
    <item-box-new title="基本信息">
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <p>出生年月</p>
          <em>{{formData.brith_date}}</em>
        </el-col>
        <el-col :span="12">
          <p>政治面貌</p>
          <em>{{formData.political_outlook}}</em>
        </el-col>
        <el-col :span="12">
          <p>进公司日期</p>
          <em>{{formData.join_date}}</em>
        </el-col>
        <el-col :span="12">
          <p>拟退休日期</p>
          <em>{{formData.retire_date}}</em>
        </el-col>
        <el-col :span="12">
          <p>健康情况</p>
          <em>{{formData.health}}</em>
        </el-col>
        <el-col :span="12">
          <p>原职位</p>
          <em>{{formData.old_position}}</em>
        </el-col>
        <el-col :span="12">
          <p>拟返聘职位</p>
          <em>{{formData.position}}</em>
        </el-col>
        <el-col :span="12" v-if="!(flowFields.result === 3)">
          <p>办理结果</p>
          <em v-if="flowFields.result === 1">{{formData.result}}</em>
          <el-input clearable v-model="formData.result" v-if="flowFields.result === 2" @change="changeInputContent"></el-input>
        </el-col>
        <el-col :span="12"  v-if="!(flowFields.is_agreement === 3)">
          <p>是否签订返聘协议</p>
          <em v-if="flowFields.is_agreement === 1">{{formData.is_agreement_name}}</em>
          <el-select v-model="formData.is_competent" v-if="flowFields.is_agreement === 2" @change="changeInputContent" style="width:100%" placeholder="请选择是否签订返聘协议">
            <el-option
        ...
        <el-col :span="12" v-if="!(flowFields.rehire_date === 3)">
          <p>返聘开始期限</p>
          <em v-if="flowFields.rehire_date === 1">{{formData.rehire_date}}</em>
          <el-date-picker
            v-if="flowFields.rehire_date === 2"
        ...
        <el-col :span="12" v-if="!(flowFields.rehire_end_date === 3)">
          <p>返聘结束期限</p>
          <em v-if="flowFields.rehire_end_date === 1">{{formData.rehire_end_date}}</em>
          <el-date-picker
            v-if="flowFields.rehire_end_date === 2"
            @change="changeInputContent"
            value-format="yyyy-MM-dd"
            type="date"
            placeholder="选择结束期限"
            v-model="formData.rehire_end_date">
          </el-date-picker>
        </el-col>
      </el-row>
  ...
  methods: {
    ...
    changeInputContent() {
      if(this.formData.state) {
        ...
        this.$client.EditEmployRetirement(params).then((req) => {})
      }
    },
  }
}
