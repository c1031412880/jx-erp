  ...
  <div class="work-contact-audit" v-loading="loading">
    ...
    <item-box-new title="基本信息">
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <p>手机号</p>
          <em>{{formData.phone}}</em>
        </el-col>
        <!-- <el-col :span="24">
          <p>反馈描述</p>
          <el-input autosize type="textarea" class="richText" disabled v-model="formData.remark"></el-input>
        </el-col> -->
        <el-col :span="24" v-if="formData.rich_text">
          <p>反馈内容详情</p>
          <div class="content-html" v-html="formData.rich_text"></div>
        </el-col>
        <el-col :span="12" v-if="!(flowFields.plan_finish_date === 3)">
          <p>厂家预计完成时间</p>
          <em v-if="flowFields.plan_finish_date === 1">{{formData.plan_finish_date}}</em>
          <el-date-picker
            value-format="yyyy-MM-dd"
            type="date"
            placeholder="选择日期"
            v-model="formData.plan_finish_date"
            @change="changeInputContent"
            v-if="flowFields.plan_finish_date === 2"
            style="width: 100%;">
          </el-date-picker>
        </el-col>
        <el-col :span="24" v-if="!(flowFields.replay_content === 3)">
          <p>厂家回复</p>
          <el-input autosize type="textarea" class="richText" v-if="flowFields.replay_content === 1" disabled v-model="formData.replay_content"></el-input>
          <el-input v-if="flowFields.replay_content === 2" v-model="formData.replay_content" @change="changeInputContent" autosize type="textarea"></el-input>
        </el-col>
      </el-row>
...
export default {
  name: "rationalization-feedback-audit",
  components: {
    itemBoxNew,
    titleBoxNew,
    imageFileUpload,
  ...
  methods: {
    ...
    changeInputContent() {
      if(this.formData.state) {
        ...
        this.$client.UpdateCustomSuggestion(params).then((req) => {})
      }
    },
  }
}
