    ...
    <el-row :gutter="20">
      ...
      <el-col :span="24" class="table-box">
        ...
        >
          ...
          <template v-for="jtem in ['rent_date', 'return_date']" slot-scope="scope" :slot="jtem">
            <el-date-picker
              v-model="scope.data[jtem]"
          ...
          <template slot-scope="scope" slot="overtime_cost">
            <el-input-number
              v-model="scope.data.overtime_cost"
              :min="0"
              placeholder="请输入"
              :controls="false"
              style="width: 100%"
    ...
    outData() {
      return new Promise((resolve, reject) => {
        this.$refs.form.validate(valid => {
          ...
          let dateValid = this.form.details.some(item => {
            return new Date(item.return_date) < new Date(item.rent_date)
          })
          if (bool) {
            reject({
