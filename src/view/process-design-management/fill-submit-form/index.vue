...
import mockData from '../form-design/mockData'
...
export default {
  ...
  data () {
    const confGlobal = this.$route.query.formData || mockData.formData
  ...
  methods: {
        ...
        for (let key in submitFormData) {
         this.confGlobal.fields.forEach((item,index) => {
           if(item.vModel == key) {
            ...
            //      signalFormObj[item.formId] = [];
            //     //  console.log(submitFormData[item.vModel])

            //      submitFormData[item.vModel].forEach((secodenItem, zIndex)=>{
            //        for (let childrenKey in secodenItem) {
            //         //  console.log(childrenKey)
            //          let childrenFormObj = {};
            //          if(childrenKey == j.vModel) {
            //           //  console.log(JSON.stringify(submitFormData[item.vModel]));
            //           //  let sss = submitFormData[item.vModel]
            //           //  sss.forEach((jItem, jIndex) => {
            //           //    console.log(jItem)
            //           //  childrenFormObj[j.formId] = jItem[childrenKey]
            //           //  childrenFormObj.lable_name = j.label || '';
            //           //  })
            //          }
            //           signalFormObj[item.formId].push(childrenFormObj)
            //        }
            //      })
    ...
    submitFormData(formDataList){
      let content = {
        ...
        //   }
        // })
        this.submitFormToProcess(content);
    },
    //表单数据发起流程
    submitFormToProcess(val){
      let objContent={
        form_id: '',
        condition: []
      }
      val.info.forEach(item => {
        let conditionObj = {}
        conditionObj.form_id = item.form_id
        conditionObj.value = item.data
        objContent.condition.push(conditionObj)
      });
      objContent.form_id = this.formId
      console.log("objContent",objContent)

      // get_flow_struct_data(objContent).then(req => {
      //   console.log(req)
      //   if (req.head.result === 200) {
      //     this.$router.push({name:'step',params: { structData: req.content.data }})
      //   } else {
  ...
  render (h) {
    if (this.isRund) {
      ...
      this.confGlobal.fields.forEach(item => {
        model[item.vModel] = item.defaultValue || undefined
      })
      this[this.confGlobal.formModel] = model
      this.isRund = false
    }
    const renderSic = <div class="preview-container" style={'width:' + this.containerWidth + '%;'}>
