<template>
  <div class="receipt-manage">
    <sub-req
      ref="head"
      @on-receipt="
        addDocument()
        isSet = true
      "
      @on-ok="onSubmit"
    ></sub-req>
    <tr-table
      ref="table"
      :selectionShow="true"
      :data="data"
      :tableHeaderList="canRendererTableHeader"
      :TableHeight="TableHeight"
      :loading="loading"
      :frontFixedNum="frontFixedNum"
      :backFixedNum="backFixedNum"
    >
      <template slot="operation" slot-scope="scope">
        <el-button
          type="text"
          v-if="(scope.data.state === 3 && scope.data.commit !== 1) || scope.data.state === 6"
          @click="onStateSet(scope.data, true)"
        >
          编辑
        </el-button>
        <el-button type="text" v-else @click="openApply(scope.data)">查看</el-button>
        <el-button
          type="text"
          v-if="scope.data.state === 3 || scope.data.state === 6"
          @click="onStateVoid(scope.data)"
        >
          作废
        </el-button>
        <el-button
          type="text"
          v-if="scope.data.state === 5 || scope.data.state === 0"
          @click="onStateDel(scope.data)"
        >
          删除
        </el-button>
      </template>
      <template slot="state_name" slot-scope="scope">
        <span :style="scope.data.state | outType">{{ scope.data.state_name }}</span>
      </template>
    </tr-table>
    <el-pagination
      style="margin: 5px 0; display: flex; justify-content: flex-end"
      @current-change="handleCurrentChange"
      @size-change="handleSizeChange"
      ref="paginationH"
      :current-page="pageInfo.pageIndex"
      :page-sizes="[10, 20, 30, 40, 50, 100]"
      :page-size="pageInfo.pageSize"
      layout="total,sizes, prev, pager, next, jumper"
      :total="pageInfo.pageCount"
    ></el-pagination>
    <custom-setting
      :dialogBool.sync="customSettingVialogVisible"
      :customSettingList="customSettingList"
      :canRendererTableHeader="canRendererTableHeader"
      :customSettingShowList="customSettingShowList"
      :frontFixedNum="frontFixedNum"
      :backFixedNum="backFixedNum"
    ></custom-setting>
    <register-dialog
      :isSet="isSet"
      ref="registerDialog"
      :dialogBool.sync="registerDialogShow"
      @on-ok="registerDialogOk"
      @on-save="loadData"
    ></register-dialog>
  </div>
</template>

<script type="text/ecmascript-6">
import subReq from './component/sub-req'
import registerDialog from './component/register-dialog'

export default {
  name: "receipt-manage",
  components: {subReq, registerDialog},
  data() {
    return {
      data: [],
      TableHeight: 400,
      customSettingList:[
        {
          id: 1,
          label: "文件标题",
          prop: "title",
          width: "200",
          align: "center",
          signIndex:0,
          sortable:false
        },
        {
          id: 2,
          label: "来文字号",
          prop: "font_number",
          width: "160",
          align: "center",
          signIndex:1,
          sortable:false
        },
        {
          id: 3,
          label: "状态",
          prop: "state_name",
          width: "120",
          align: "center",
          signIndex:2,
          sortable:false
        },
        {
          id: 4,
          label: "创建人姓名",
          prop: "register_name",
          width: "120",
          align: "center",
          signIndex:3,
          sortable:false
        },
        {
          id: 5,
          label: "保密密级",
          prop: "secret_name",
          width: "130",
          align: "center",
          signIndex:4,
          sortable:false
        },
        {
          id: 6,
          label: "来文机关",
          prop: "organ",
          width: "120",
          align: "center",
          signIndex:5,
          sortable:false
        },
        {
          id: 7,
          label: "来文日期",
          prop: "accept_date",
          width: "160",
          align: "center",
          signIndex:6,
          sortable:false
        },
        {
          id: 8,
          label: "主送单位",
          prop: "main_department_name",
          width: "160",
          align: "center",
          signIndex:7,
          sortable:false
        },
        {
          id: 9,
          label: "主送人员",
          prop: "main_person_names",
          width: "160",
          align: "center",
          signIndex:8,
          sortable:false
        },
        {
          id: 10,
          label: "抄送单位",
          prop: "copy_department_names",
          width: "160",
          align: "center",
          signIndex:9,
          sortable:false
        },
        {
          id: 11,
          label: "抄送人员",
          prop: "copy_person_names",
          width: "160",
          align: "center",
          signIndex:10,
          sortable:false
        },
        {
          id: 12,
          label: "操作",
          prop: "operation",
          width: "160",
          align: "center",
          signIndex:11,
          sortable:false
        },
      ],
      frontFixedNum:2, //前几行固定
      backFixedNum:1, //后几行固定
      canRendererTableHeader: null,
      customSettingShowList: null,
      loading: false,
      pageInfo: {
        pageIndex: 1,
        pageSize: 20,
        pageCount: 0
      },
      registerDialogShow: false,
      isSet: false,
      customSettingVialogVisible: false,
      titlesId:'', //当前表头的id
      menu_code:'',
    }
  },
  mounted() {
    this.$nextTick(() => {
      this.menu_code = this.$route.meta.code + '-2';
      this.reqTitleTable()
      this.setHead('initialize');
      this.loadData()
      window.eventOn.$on('view-resize' + this.$route.fullPath, () => {
        this.setHead()
      })
    })
  },
  filters: {
    outType(type) {
      let color = '#409EFF'
      switch (type) {
        case 0:
          color = '#E6A23C'
          break
        case 2:
          color = '#E6A23C'
          break
        case 3:
          color = '#F56C6C'
          break
        case 5:
          color = '#F56C6C'
          break
        case 4:
          color = '#67C23A'
          break
      }
      return {color: color}
    }
  },
  methods: {
    // 删除
    onStateDel(info) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$client.DeleteDocumentAccept({context: [info.id]}).then(req => {
          this.loadData()
          if (req.head.result === '200') {
            this.$message({
              message: req.head.describle,
              type: 'success'
            });
          } else {
            this.$message.error(req.head.describle);
          }
        })
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消删除'
        });
      });
    },
    // 作废
    onStateVoid(info) {
      this.$confirm('此操作将作废该记录, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$client.UpdateScrapDocumentAccept({context: [info.id]}).then(req => {
          this.loadData()
          if (req.head.result === '200') {
            this.$message({
              message: req.head.describle,
              type: 'success'
            });
          } else {
            this.$message.error(req.head.describle);
          }
        })
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消作废'
        });
      });
    },
    onSubmit(form) {
      this.form = form
      this.pageInfo.pageIndex = 1
      this.loadData()
    },
    // 收文登记
    addDocument() {
      this.registerDialogShow = true
      setTimeout(() => {
        this.$refs.registerDialog.getDraft()
      }, 100)
    },
    // 数据回显
    onStateSet(item, isSet = false) {
      this.registerDialogShow = true
      this.isSet = isSet
      setTimeout(() => {
        this.$refs.registerDialog.setData(item)
      }, 100)
    },
    // 审批查看
    openApply(val) {
      let detail_id = 40, title = '收文审批'
      this.$router.push({
        path: `/news-details/${val.id}/${title}/${detail_id}`
      })
    },
    // 提交
    registerDialogOk(form) {
      // if (!form.font || !form.number) {
      //   this.$message.warning('请填写来文字号');
      //   return false
      // }
      if (!form.organ) {
        this.$message.warning('请填写公来文机关');
        return false
      }
      if (!form.accept_date) {
        this.$message.warning('请填写来文日期');
        return false
      }
      // if (!form.secret) {
      //   this.$message.warning('请填写保密等级');
      //   return false
      // }
      // if (!form.type) {
      //   this.$message.warning('请填写来文类型');
      //   return false
      // }
      if (!form.title) {
        this.$message.warning('请填写文件标题');
        return false
      }
      if (!form.register_id) {
        this.$message.warning('请选择创建人');
        return false
      }
      if (!form.department_id) {
        this.$message.warning('请选择创建人部门');
        return false
      }
      if (!form.main_department) {
        this.$message.warning('请选择主送单位');
        return false
      }
      // if (!form.main_persons.length) {
      //   this.$message.warning('请选择主送人员');
      //   return false
      // }
      if (!form.flow_id) {
        this.$message.warning('请选择收文流程');
        return false
      }
      if (!form.content) {
        this.$message.warning('请上传正文');
        return false
      }
      if (form.id) {
        this.$client.UpdateDocumentAccept({context: form}).then(req => {
          this.loadData()
          if (req.head.result === '200') {
            let form = {
              document_id: req.context.id,
              flow_id: req.context.flow_id,
              state: 0,
              state_name: '草稿'
            }
          this.registerDialogShow = false
          this.isSet = false
          } else {
            this.$message.error(req.head.describle);
          }
        })
      } else {
        this.$client.AddDocumentAccept({context: form}).then(req => {
          this.loadData()
          if (req.head.result === '200') {
            let form = {
              document_id: req.context.id,
              flow_id: req.context.flow_id,
              state: 0,
              state_name: '草稿'
            }
            this.registerDialogShow = false
            this.isSet = false
          } else {
            this.$message.error(req.head.describle);
          }
        })
      }
    },
    loadData() {
      this.loading = true
      this.form.page_index = this.pageInfo.pageIndex
      this.form.page_size = this.pageInfo.pageSize
      this.$client.GetByPageDocumentAccept(this.form).then(req => {
        this.loading = false
        if (req.head.result === '200') {
          this.data = req.context.list
          this.pageInfo.pageCount = req.context.total
        } else {
          this.data = []
          this.pageInfo.pageCount = 0
        }
      })
    },
    // 获取表头
    reqTitleTable() {
      this.$httpTableHeader.reqTableHeader(this.menu_code,this.customSettingList).then(res => {
        this.titlesId = res.titlesId;
        this.canRendererTableHeader = res.canRendererTableHeader;
        this.customSettingList = res.customSettingList;
        this.customSettingShowList = res.customSettingShowList;
        this.$forceUpdate();
      }).catch(err => {})
    },
    updateTableHeader(newestTableHeaderList) {
      this.$httpTableHeader.saveTableHeader(this.titlesId,this.menu_code,newestTableHeaderList).then(res => {

        this.reqTitleTable()
      }).catch(err => {

      })
    },
    openSettingPageEvent() {
      this.customSettingVialogVisible = true;
    },
    // 分页切换事件
    handleCurrentChange(idx) {
      this.pageInfo.pageIndex = idx
      this.loadData()
    },
    // 切换每页条数
    handleSizeChange(sizes) {
      this.pageInfo.pageIndex = 1;
      this.pageInfo.pageSize = sizes;
      this.pageInfo.pageCount = 0;
      this.loadData();
    },
// 计算表格高度
    setHead(type) {
      if(type == 'initialize') {
        this.canRendererTableHeader = this.customSettingList;
      }
      setTimeout(() => {
        let maxh = this.$el.offsetHeight
        let th = this.$refs.head.$el.offsetHeight
        this.TableHeight = maxh - (th) - 30
        console.log(this.TableHeight)
      }, 60)
    }
  }
}
</script>

<style lang="stylus" rel="stylesheet/stylus" scoped>
.receipt-manage{
  width 100%
  height 100%
}
</style>
