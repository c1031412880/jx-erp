<template>
  <div class="enclosure">
    ...
    <el-table style="width: 100%" :data="data"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="size" align="center" label="文件大小"></el-table-column>
  ...
  methods: {
    ...
    openDocument(item) {
      ...
      if(file_extension == 'rar' || file_extension == 'zip') {
        ...
        url = newHttpConfig.previewFile + '/index.html' + `?isEdit=true&user=${userInfo.user_id}&token=${token}&file_id=${item.file_id}&type=${file_extension}`
      }
      window.open(url)
    },
    onDow(val) {
