...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.$client.MaintSummaryByDriverMaintManageAnalysis(this.form).then(req => {
        this.loading = false
        if (req.head.result === '200') {
          ...
          if(info && info.length) {
            let obj = {}
            info.forEach(item => {
              obj = Object.assign({}, item.datas[0])
          ...
          this.rowspanObj = this.$handleTableSpan(this.mergekeys, this.data, 'driver_name', true)
    ...
    exportTableData() {
      this.$client.ExportByDriverMaintManageAnalysis(this.form).then(req => {
        if (req.head.result === '200') {
