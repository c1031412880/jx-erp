<template>
  <div class="safety-meeting-record">
    <sub-req
      ref="Head"
      @on-ok="onSubmit"
      @on-del="onDelRowData"
    ...
    >
      ...
      <template slot-scope="scope" slot="attendance">
        <p>{{ scope.data.attendance }}</p>
        <el-button @click="onCheckRowRecord(scope.data)" type="text" size="mini">
          查看签到记录
        </el-button>
      </template>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.$client
        .GetByPageSafeMeetingMain(params)
        .then(res => {
    ...
    saveAddEditRowData(form) {
      ...
      let obj = {
        add: 'InsertSafeMeetingMain',
        edit: 'UpdateSafeMeetingMain'
      }
    ...
    onDelRowData(type, row) {
      ...
      this.$client.DeleteSafeMeetingMain(params).then(res => {
        if (res.head.result == '200') {
          this.loadData()
          this.$message({
            showClose: true,
    ...
    onExportRowData() {
      ...
      params.page_index = this.pageInfo.pageIndex
      params.page_size = this.pageInfo.pageSize
      this.$client.ExportSafeMeetingMain(params).then(res => {
        if (res.head.result == '200') {
