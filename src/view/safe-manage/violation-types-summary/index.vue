<template>
  <div class="violation-types-summary">
...
export default {
  ...
  methods: {
    ...
    onRadio(form) {
      this.form = JSON.parse(JSON.stringify(form))
      this.setTime()
      this.GetViolationTypeSummaryByType()
    },
    setTime() {
    ...
    loadData() {
      this.GetViolationTypeSummaryPie()
      this.GetViolationTypeSummaryBar()
      this.GetViolationTypeSummaryByType()
    },
    // 饼图 
    GetViolationTypeSummaryPie() {
      this.$client.GetViolationTypeSummaryPie(this.form).then(res => {
    ...
    // 柱状图 
    GetViolationTypeSummaryBar() {
      this.$client.GetViolationTypeSummaryBar(this.form).then(res => {
        if (res.head.result === '200') {
    ...
    // 统计列表 
    GetViolationTypeSummaryByType() {
      this.$client.GetViolationTypeSummaryByType(this.form).then(res => {
        if (res.head.result === '200') {
    ...
    exportFile() {
      ...
      this.exportToExcel(table_dom,'违章类型汇总')
    },
    // 计算表格高度 和初始化表头
    setHead() {
      setTimeout(() => {
