<template>
  <div class="accident-nature">
    <div class="chart-container" id="accident-nature-chart"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.$client.GetAccidentNatureAnalysis(params).then(res => {
    ...
    initChart() {
      this.$nextTick(() => {
        const chartBox = document.getElementById('accident-nature-chart')
        if (!chartBox) {
          console.warn('事故性质图表容器不存在')
          return
        }
