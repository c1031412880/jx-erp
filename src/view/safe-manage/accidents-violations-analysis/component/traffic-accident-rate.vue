<template>
  <div class="traffic-accident-rate">
    <tr-table
      ref="table"
      :selectionShow="false"
      :indexShow="false"
    ...
    >
      <template slot-scope="scope" slot="color">
        <!-- <div :style="{'background-color': scope.data.color,height:'20px'}"> -->
...
import common from '@/mixin/exportToExcel'
  export default {
    ...
    methods: {
      ...
      loadData(form) {
        this.loading = true
        this.$client.GetAccidentRateTotal({context:form}).then(res => {
          this.loading = false
      ...
      exportFile(form) {
        ...
        // this.exportToExcel(table_dom,'行车事故率')
        this.$client.ExportAccidentsReport({context:form})
        .then(res => {
          if(res.head.result == '200') {
