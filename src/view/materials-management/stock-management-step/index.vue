<template>
  <div class="tyre-maintain">
    ...
    <template slot-scope="scope" slot="enter_type">
      ...
      <span v-else-if="scope.data.enter_type=='TLRK'">退料入库</span>
    ...
    <template slot-scope="scope" slot="operation" >
        ...
        <!-- <el-button type="text" size="mini" v-if="$isPowerShow('details')"  @click="goEdit(scope.data.id)">查看</el-button> -->
        ...
        <!-- <el-button type="text" size="mini" v-else @click="checkStatus(scope.data,'3')">反审核</el-button> -->
          ...
          <!-- <el-button v-if="scope.data.state!=2&&$isPowerShow('audit')" @click="open(scope.data,'2')" slot="reference" type="text" size="mini" style="margin-left: 10px;" >审核</el-button> -->
          <!-- <el-button slot="reference" @click="open(scope.data,'3')" v-if="scope.data.state==2 && scope.data.enter_type !='PYRK' && $isPowerShow('unaudit')" type="text" size="mini" style="margin-left: 10px;" >反审核</el-button> -->
        <!-- <el-popconfirm
...
export default {
  ...
  reqTitleTable() {
    ...
    checkDetail(id) {
      this.$router.push({
        path: `/news-details/${id}/${'物资入库单'}/${1450}`
      })
    },
    goEdit(data) {
      // this.$router.push({
