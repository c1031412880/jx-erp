  ...
  >
          ...
          >
            ...
            <!-- <template slot-scope="scope" slot="cost_price">
              <el-input
                ...
                oninput="value=value.replace(/[^\d.-]/g,'')"
                @blur="scope.data.cost_price = $event.target.value"
                @change="priceChange(scope.data,scope.index)"
                style="width: 100px"
              ></el-input>
            </template>
    ...
    <el-dialog
      ...
      <materials-box 
        ...
        @on-load="getwuzi"
      ></materials-box>
      <!-- <materials-box ref="materials-box" @on-save="saveMaterials"></materials-box> -->
    </el-dialog>
  </el-dialog>
</template>
...
import setInfoDialog from "../../materials-manage/component/set-info-dialog"
  ...
  data() {
    return {
      ...
      form: {
        ...
        enter_date: '',
        enter_type: "CGRK",
        enter_date: formatDate(new Date(),'yyyy-MM-dd'),
        relate_num: "",
        storage_id: "",
        buyer_id: "",
        remark: "",
  ...
  methods: {
    ...
    returnSubmission(){
      this.outData().then(res => {
        ...
        this.$client.setRecordEnterHouse(parmas)
        .then((res) => {
    ...
    //   setTimeout(() => {
    //     this.$refs.setOrganizationInfo.ruleForm.main_id = this.tireID
    //   }, 60);
    // },
    // 添加
    saveAddEdit(form) {
    ...
    GetDraftRecordFlowManage() {
      this.$client.GetDraftRecordFlowManage({type: 1450}).then(res => {
        if(res.head.result == '200') {
          ...
          if(!!info) {
            ...
            this.TableHeight = this.tableData.length === 0 ? 150 :  this.tableData.length * 50 + 100
          }
          this.getWarehouseperson1(this.form.house_id);
        }else{
          this.$message({
    ...
    ok(type) {
      ...
      this.outData().then(res => {
        ...
        this.$client.AddByFlowEnterHouse({context: {
          form_data: formData,
          step_data: step_data,
        }}).then(req => {
          this.updateLoading = false
          if (req.head.result === "200") {
            this.$emit('on-load')
            if (type == 1) {
              this.temporarySave(deepCopy(formData))
              this.dialogClose()
            } else {
              this.initData()
            }
    ...
    initData() {
      let form =  {
        enter_num: '',
        enter_date: '',
        enter_type: this.form.enter_type,
        enter_date: this.form.enter_date,
        relate_num: "",
        storage_id: "",
        buyer_id: "",
        remark: "",
    ...
    //获取时间
    // getdate(){
    //    this.form.enter_date = formatDate(new Date())
    // },
    //供应商
    getGinsang() {
      this.$client.GetProviderPage().then((req) => {
        if (req.head.result === "200") {
          ...
          if (req.context.list.length > 0) {
            req.context.list.forEach(item => {
    ...
    priceChange(val,i) {
      ...
      //   });
      // }
      this.tableData[index].total_price =(this.tableData[index].count * this.tableData[index].price).toFixed(2);
      if(this.tableData[index].cost_price){
        this.tableData[index].cost_total_price = (this.tableData[index].count * this.tableData[index].cost_price).toFixed(2);
      }else{
        this.tableData[index].cost_total_price= ''
      }
      this.getAllfee()
      this.dateChange()
    },
    ...
    addData(info) {
      ...
      if (this.tableData.length > 0) {
        this.tableData.push({
          ...
          material_id: info ? info.id : '',
          material_name: info ? info.name : '',
          specification: info ? info.specification : '',
          measure_unit_name: info ? info.measure_unit_name : '',
