...
<script type="text/ecmascript-6">
  ...
  created() {
    ...
    ObjectSpanMethod({ row, column, rowIndex, columnIndex }) {
      if([0,1,2,3,4,5,6,7,8,20,21,22,23,24,25].indexOf(columnIndex) !== -1) {
        // 第一列的合并方法,省
    ...
    // ObjectSpanMethod({ row, column, rowIndex, columnIndex }) {
    //   if (columnIndex === 0) {
    //     if(rowIndex ===0 || row.enter_num != this.data[rowIndex-1].enter_num){
    //       let rowspan = 0;
    //       this.data.forEach(element => {
    //         if(element.enter_num === row.enter_num){
    //           rowspan ++;
    //         }
    //       });
    //       return [rowspan, 1];
...
// }
  ...
  .table-box /deep/ tr.hover-row>td.el-table__cell {
    background-color: transparent;
  }
  .table-box /deep/.el-table--striped .el-table__body tr.el-table__row--striped td.el-table__cell {
    background-color: transparent;
  }
</style>
<style>
.table-box .mo-table__row--active {
