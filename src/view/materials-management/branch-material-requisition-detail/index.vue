<template>
  <div class="branch-material-requisition-detail">
    <sub-req
      ref="head"
      @on-export="onExport"
      @on-ok="onSubmit"
...
export default {
  ...
  methods: {
    ...
    // 数据加载 - 对接GetDeptLeave接口
    loadData() {
      this.loading = true;

      try {
                 // 构建接口请求参数（server_id已在http.js中自动处理）
         const params = {
           context: {
             start_time: this.formatDateToISO(this.form.start_date, '00:00:00'),
             end_time: this.formatDateToISO(this.form.end_date, '23:59:59'),
             house_ids: this.form.hourse_id && this.form.hourse_id.length > 0 ? this.form.hourse_id : [],
        ...
        // 调用分公司物料领用分组明细报表接口
        this.$client.GetDeptVehicleLeaveInventoryReport(params)
          .then((response) => {
            this.loading = false;
          ...
          .catch((error) => {
            this.loading = false;
            console.error('GetDeptLeave接口调用失败:', error);
            this.$message({
              showClose: true,
    ...
    onExport() {
      try {
                 // 构建导出参数（server_id已在http.js中自动处理）
         const params = {
           context: {
             start_time: this.formatDateToISO(this.form.start_date, '00:00:00'),
             end_time: this.formatDateToISO(this.form.end_date, '23:59:59'),
        ...
        // 调用导出接口
        this.$client.DeptVehicleLeaveExportInventoryReport(params)
          .then((response) => {
            if (response.head && response.head.result === "200") {
              if (response.context) {
                // 如果返回文件ID，构建下载链接
     ...
     handleResize() {
       ...
       this.resizeTimer = setTimeout(() => {
         ...
         // 设置表格高度 - 参考material-receiving-and-sending-summary的做法
     setHead() {
       setTimeout(() => {
...
.branch-material-requisition-detail
  // 参考material-receiving-and-sending-summary的样式做法
  width: 100%
  height: 100%
</style>
