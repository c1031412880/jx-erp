      ...
      >
        ...
        <div class="show-flex-box-r">
          <el-form-item label="来源供应商:">
            <get-supplier-list ref="driverTree2" v-model="ruleForm.provider_id" :width="'390'" :placeholder="'请选择供应商'" :isExportMultiple="false"></get-supplier-list>
<!--            <el-input v-model="ruleForm.vendor" :width="'420'"> </el-input>-->
        ...
        <div class="show-flex-box-r">
          <el-form-item label="最新入库价（元）:" >
             <el-input-number v-model="ruleForm.price" :min="0" :width="'420'"></el-input-number>
          </el-form-item>
          <el-form-item label="税率%:" >
            <el-input-number v-model="ruleForm.tax_rate" :min="0" :width="'420'"></el-input-number>
          </el-form-item>
        ...
        <div class="show-flex-box-r">
          <el-form-item label="助记码:" >
            <el-input v-model="ruleForm.mnemonic_code " :min="0" :width="'420'"></el-input>
          </el-form-item>
          <!-- <el-form-item label="使用车型:" >
             <el-select multiple collapse-tags v-model="ruleForm.vehicle_kind_ids" placeholder="请输入车辆型号" clearable :style="{width: '100%'}">
              <el-option v-for="(item, index) in veh_typeOptions" :key="index" :label="item.name"
        ...
        <div class="show-flex-box-r">
          <el-form-item label="最低库存量:">
            <el-input
            v-model="ruleForm.min_inventory"
            :width="'420'"
            oninput="value=value.replace(/[^\d.-]/g,'')"
            @blur="ruleForm.min_inventory = $event.target.value"
            size="mini"
            clearable>
          </el-input>
          </el-form-item>
          <el-form-item label="最高库存量:">
            <el-input
              v-model="ruleForm.max_inventory"
              :width="'420'"
              oninput="value=value.replace(/[^\d.-]/g,'')"
              @blur="ruleForm.max_inventory = $event.target.value"
              size="mini"
              clearable>
            </el-input>
          </el-form-item>
        ...
        <div class="show-flex-box-r">
          <el-form-item label="安全库存量:">
            <el-input
              v-model="ruleForm.safty_inventory"
              :width="'420'"
              oninput="value=value.replace(/[^\d.-]/g,'')"
              @blur="ruleForm.safty_inventory = $event.target.value"
              size="mini"
              clearable>
            </el-input>
          </el-form-item>
          <el-form-item label="预警值:">
            <el-input
              v-model="ruleForm.warn_inventory"
              :width="'420'"
              oninput="value=value.replace(/[^\d.-]/g,'')"
              @blur="ruleForm.warn_inventory = $event.target.value"
              size="mini"
              clearable>
            </el-input>
          </el-form-item>
    ...
    <span slot="footer" class="dialog-footer">
      <el-button type="primary" @click="updatePlaybill('ruleForm',1)" size="mini">保存并继续添加</el-button>
      <el-button @click="dialogClose" size="mini">取 消</el-button>
      <el-button type="primary" @click="updatePlaybill('ruleForm',2)" size="mini">保 存</el-button>
    </span>
  </el-dialog>
</template>
...
import getSupplierList from "@/components/base/formModel/select/get-supplier-list.vue";
export default {
  components: { getMaterialsSelect,getDepartmentSelect,getSelectDictionaries,getSupplierList },
  props: {
    dialogBool: Boolean,
  ...
  methods: {
    closeData() {
      let main_id = this.ruleForm.main_id
      Object.assign(this.$data, this.$options.data());
      // this.$refs.driverTree1.onClear()
      this.ruleForm.temp_meterial ='2'
      this.ruleForm.usable ='1'
      this.ruleForm.main_id = main_id
    },
    // 获取车辆型号列表
    getVehType(){
    ...
    updatePlaybill(formName, type) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if(info.vehicle_kind_ids){
            info.vehicle_kind_ids = info.vehicle_kind_ids.toString()
          }
          this.$emit("save-add-edit", info, type, 2);
        } else {
          return false;
        }
      });
