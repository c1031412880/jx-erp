  ...
  methods: {
    ...
    copyRowInfo(form) {
      ...
      .then(res => {
        if(res.head.result == '200') {
          this.curSelecedType = 'add';
          this.informationDialogTitle = "复制出库单";
          this.informationDialogShow = true;
          this.$nextTick(() => {
            // 复制数据但将主键ID设置为0，表示新增
            ...
            copyData.id = 0;
            // 清空一些不需要复制的字段
            copyData.leave_num = ''; // 清空出库单号，让系统自动生成
            copyData.state = 1; // 设置为未审核状态
            copyData.state_name = '未审核';
            copyData.review_date = ''; // 清空审核时间
            copyData.review_name = ''; // 清空审核人
            copyData.created_date = ''; // 清空创建时间
            copyData.created_name = ''; // 清空创建人名称
            // 清空明细的一些字段
            if (copyData.details && copyData.details.length > 0) {
              copyData.details.forEach(detail => {
                detail.id = 0; // 明细ID也设置为0
                detail.batch_no = ''; // 清空批次，让系统重新生成
              });
            }
      ...
      this.$client.getDataLeaveHouse({id:form.id})
        .then(res => {
          if(res.head.result == '200') {
            ...
            this.informationDialogTitle = "出库单审批";
            this.informationDialogShow = true;
            this.$nextTick(() => {
              this.$refs.informationDialog.setData(res.context)
            })
