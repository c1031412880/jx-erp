...
export default {
  ...
  methods: {
    ...
    queryList() {
      ...
      this.$client.getMaintRepairSimpleData({
        ...
        if (res.head.result == "200") {
          this.allList = res.context.map(item => {
            let value = item.work_code
            let vehicle_name = item.vehicle_info ? item.vehicle_info.vehicle_name : null
            let label = value + "(" + vehicle_name + ")"
            return {
              value,
              vehicle_name,
              label
    ...
    onChange(val) {
      let vehicle_name = ''
      if (val) {
        vehicle_name = this.workcodeList.filter(item => item.value == val)[0].vehicle_name
      }
      this.$emit('on-change', vehicle_name)
      this.$emit('input', val)
    }
  },
}
