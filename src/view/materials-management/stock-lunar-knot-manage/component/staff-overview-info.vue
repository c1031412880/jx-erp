...
export default {
  ...
  methods: {
    ...
    reqStaffTypeCount(backup_date) {
      let parmas = {}
      if(!!backup_date) {
        parmas.backup_date = backup_date
      }
      this.$client.GetCountTimerBackup(parmas)
      .then(res => {
        // console.log(res)
    ...
    configStaffCount(staffTypeCountList) {
      // alert(111)
      const typelist = Object.keys(staffTypeCountList)
      console.log(Object.keys(staffTypeCountList) )
      typelist.forEach((item,index) => {
        this.data.forEach((zItem,zIndex) => {
          if(item== zItem.type) {
            zItem.numberCount = staffTypeCountList[item]
          }
        });
      });
    },
