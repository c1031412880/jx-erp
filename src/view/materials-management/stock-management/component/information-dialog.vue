    ...
    <div class="el-dialog-div-content">
      ...
      >
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="制单人:" prop="vehicle_id">
      ...
      <el-tag type="info">总数量:{{count_count}}</el-tag>
      <el-tag type="info">总金额:{{price_count}}</el-tag>
      <div class="table">
        <tr-table
          :selectionShow="true"
          :data="
        ...
        >
          ...
          <template slot-scope="scope" slot="barcode">
            <span>{{ scope.data.barcode || '--' }}</span>
          </template>
...
export default {
  ...
  watch: {
    dialogBool(bool) {
      if (bool === false) {
        ...
        this.form.storage_id = this.userInfo.user_id
  ...
  methods: {
    ...
    setData(info) {
      ...
      this.userInfo.user_name = info.created_name
    },
    ...
    priceChange(val) {
      ...
      this.tableData[i].total_price =
        ((this.tableData[i].count || 0) * this.tableData[i].price).toFixed(2);
        if(this.tableData[i].cost_price){
        this.tableData[i].cost_total_price =
        ((this.tableData[i].count || 0) * this.tableData[i].cost_price).toFixed(2);
        }else{
          this.tableData[i].cost_total_price= ''
        }
     this.updateCount()
    ...
    updateCount() {
      ...
      for(let i=0;i<this.tableData.length;i++) {
        this.price_count += parseFloat(this.tableData[i].total_price) || 0
        this.count_count += parseFloat(this.tableData[i].count) || 0
      }
      this.price_count = this.price_count.toFixed(2)
    },
    unique(arr) {
      // 根据唯一标识no来对数组进行过滤
      ...
      if (query !== "") {
        ...
        setTimeout(() => {
          ...
          for (let i = 0; i < optionAll.length; i++) {
            if (optionAll[i].name.toLowerCase().indexOf(query.toLowerCase()) > -1 ||
              (optionAll[i].mnemonic_code ? optionAll[i].mnemonic_code.toLowerCase().indexOf(query.toLowerCase()) > -1 : false)) {
      ...
      for (let o = 0; o < this.tableData.length; o++) {
        for (let j = o + 1; j < this.tableData.length; j++) {
          if (
            ...
            this.tableData[o].material_id !== null && this.tableData[o].material_id !== ''
          ) {
            Object.keys(
      ...
      this.$client.GetSingleMaterial({
        ...
        if (req.head.result === "200") {
          ...
          if (content) {
            ...
            this.tableData[i].barcode = content.barcode || '';
          }
        }
      })
    ...
    tableSelectRow(row) {
      console.log('行选择');
      console.log(row);
      let obj = this.wuziOptionAllMap[row.material_id]
      if (obj) {
        this.remoteWuzi = obj.code
      }
      // for(let i=0;i<this.wuziOptions.length;i++) {
